<script lang="ts">
  import { calculatePnL } from "~/utils/functions";
  ...
  $: formatData = holdingTokenData
    ?.filter(
      (item: any) =>
        Number(item?.amount || 0) * Number(item?.price?.price || 0) > 0,
    )
    ?.map((item: any) => {
      return {
        ...item,
        pnl: calculatePnL(
          Number(item?.profit?.realizedProfit || 0),
        ...
        case "7D":
          return (
            ...
            dayjs().subtract(8, "day").valueOf() <
              dayjs(item?.profit?.latestTrade).valueOf()
          );
        case "30D":
          return (
            item?.profit &&
...
>
  <div class="flex md:flex-row flex-col gap-4 justify-between">
    <div class="font-medium text-2xl">{$t("Open Positions")}</div>
    ...
    <span slot="desktop">
      {#if ((holdingTokenData && holdingTokenData.length === 0) || (formatData && formatData.length === 0)) && !isLoading}
        <div class="bg_f4f5f8 grid grid-cols-7 sticky top-0 z-9">
          ...
          <div
            class="py-3 pr-3 rounded-tr-[10px] flex items-center justify-end"
          >
            <div
              class="flex items-center gap-2 cursor-pointer w-max"
      ...
      {:else}
        <VirtualList
          ...
          width="100%"
          height={formatData.length < 10
            ? formatData.length * 70 + 1 + 40
            : 405}
        ...
        >
          ...
          >
            ...
            {:else}
                  ...
                  <div class="relative">
                    ...
                    {#if ["EVM", "MOVE", "BUNDLE"].includes($typeWallet)}
                      <div class="absolute -top-2 -right-1">
                        <img
                          src={detectedChain(formatData[index - 1]?.chain)
                            ?.logo}
                          alt=""
                          width="15"
                          height="15"
              ...
              >
                <div
                  class="h-full flex items-center justify-end font-medium text-sm text_00000099"
                >
                  $<TooltipNumber
              ...
              >
                <div class="flex flex-col gap-1">
                  ...
                  >
                    ...
                    {/if}
                    <TooltipNumber
                      number={formatData[index - 1]?.amount || 0}
                      type="balance"
                      personalValue
                    />
                  </div>
            ...
            <div class="flex justify-between items-start">
              ...
              <div class="flex flex-col gap-1">
                ...
                >
                  ...
                  {/if}
                  <TooltipNumber
                    number={formatData[index]?.amount || 0}
                    type="balance"
                    personalValue
                  />
                </div>
