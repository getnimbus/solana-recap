<script lang="ts">
  ...
  import { isNimbusVersion, timeFrame } from "~/utils/constants";
  ...
  $: formatData = holdingTokenData
    ?.filter(
      (item: any) =>
        Number(item?.avgCost || 0) > 0 &&
        Number(item?.amount || 0) > 0 &&
        Number(item?.price?.price || 0) === 0,
    )
    ?.map((item) => {
      return {
        ...item,
  ...
  const handlePercentRealizedProfit = (data: any) => {
                  ...
                  {:else}
                    <div class="flex flex-col">
                      ...
                      >
                        <TooltipNumber
                          number={Math.abs(
                            formatData[index - 1].realizedProfit,
                          )}
                          type="value"
                          personalValue
                        />
