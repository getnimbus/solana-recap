    ...
    if (data.length !== 0) {
      ...
      if (formatData && formatData.length !== 0) {
        option = {
          ...
          visualMap: {
            min: 0,
            max: Number(maxHistorical.value),
            calculable: true,
            type: "piecewise",
            orient: "horizontal",
            top: 0,
...
<div
  class={`rounded-[20px] py-4 relative lg:px-3 xl:border border_0000001a ${
    darkMode ? "lg:bg-[#222222]" : "bg-[#fff]"
  }`}
>
  <div class="flex flex-col gap-1 xl:pl-6">
    <div class="font-medium flex justify-start w-max text-xl">
      Total gas fee paid
    </div>
