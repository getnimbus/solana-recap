<script lang="ts">
  ...
  import { getChangePercent, getPositionInRange } from "~/utils/chart-utils";
        ...
        series: [
          {
            ...
            data: Object.keys(riskGroup).map((riskType) => {
              return {
                ...
                value: sumBy(
                  riskGroup[riskType],
                  (item) => Number(item?.amount || 0) * Number(item?.rate || 0),
                ),
              };
            }),
          },
  ...
  $: volatilityCompareAvg = getPositionInRange(
    Number(data?.base?.volatility || 0),
  ...
  $: drawDownCompareAvg = getPositionInRange(
    Number(data?.base?.drawDown || 0),
