<script lang="ts">
  ...
  import { typeList, isNimbusVersion } from "~/utils/constants";
  ...
  const getSectorGrowth = async (dataPersonalizeTag) => {
    ...
    try {
      const response: any = await nimbus
        .get(
          `/v2/analysis/${$wallet}/sector-growth?chain=${$chain}&fromDate=${""}&toDate=${""}`,
        )
        .then((res) => {
          return {
            result: res.data,
...
<div class="border border_0000001a rounded-[20px] p-6">
  <div class="flex flex-col gap-4">
    <div class="text-xl font-medium">Sector Growth</div>
    <div
