<script lang="ts">
  ...
  //             (prev, item) =>
  //               prev + Number(item?.amount || 0) * Number(item?.price?.price),
  //             0
  //           );
  //         }
  //       }),
      ...
      const inflowData = [
        {
          ...
          data: groupByDirectionData.map((item) => [
            ...
            sumBy(
              item.changes.inflow,
              (row) => Number(row?.amount || 0) * Number(row.price?.price || 0),
            ),
          ]),
        },
      ];
...
<AnalyticSection {address} {isShowSoon}>
  <span slot="title">
    <div class="flex justify-start font-medium text-2xl">Money flow</div>
  </span>
