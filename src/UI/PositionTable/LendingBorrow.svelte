<script lang="ts">
  import LendingBorrowItem from "./TableItem/LendingBorrowItem.svelte";
  import TooltipNumber from "~/components/TooltipNumber.svelte";

  export let positions;
  export let position;
  export let sum;
  export let sum_claimable;
</script>

<div class="flex flex-col gap-5">
  <div class="flex justify-between items-end">
    <div class="text-xl font-semibold">{position}</div>
    <div class="flex flex-col gap-1">
      <div class="text-3xl font-semibold flex justify-end">
        $<TooltipNumber number={Math.abs(sum)} type="balance" />
      </div>
      <!-- <div class="text-lg font-medium text-gray-600 flex justify-end gap-1">
        Claimable:
        <span>
          $<TooltipNumber number={sum_claimable} type="balance" />
        </span>
      </div> -->
    </div>
  </div>

  <div class="border border_0000000d rounded-[10px] overflow-x-auto">
    <table class="table-auto xl:w-full w-[1200px]">
      <thead>
        <tr class="bg_f4f5f8">
          <th
            class="pl-3 py-3 xl:static xl:bg-transparent sticky left-0 z-10 bg_f4f5f8"
          >
            <div class="text-xs font-semibold uppercase text-left">Pool</div>
          </th>

          <th class="py-3">
            <div class="text-right text-xs font-semibold uppercase">Amount</div>
          </th>

          <th class="py-3">
            <div class="text-right text-xs font-semibold uppercase">Entry</div>
          </th>

          <!-- <th class="py-3">
            <div class="text-right text-xs font-semibold  uppercase">
              Entry Time
            </div>
          </th>

          <th class="py-3">
            <div class="text-right text-xs font-semibold  uppercase">
              APY (%)
            </div>
          </th> -->

          <th class="py-3">
            <div class="text-xs font-semibold uppercase text-right">Value</div>
          </th>

          <th class="pr-3 py-3">
            <div class="text-xs font-semibold uppercase text-right">
              Profit & Loss
            </div>
          </th>
        </tr>
      </thead>
      <tbody>
        {#each positions as item}
          <LendingBorrowItem data={item} />
        {/each}
      </tbody>
    </table>
  </div>
</div>

<style>
</style>
