<script lang="ts">
  ...
  const formatDataHoldingNFT = () => {
    tokens = selectedNft?.tokens;

    data = {
      ...selectedNft,
      marketPrice: Number(selectedNft?.marketPrice || 0),
      current_native_value:
        Number(selectedNft?.floorPrice) * selectedNft?.tokens?.length,
      current_value:
        Number(selectedNft?.floorPrice) *
        Number(selectedNft?.marketPrice) *
        selectedNft?.tokens?.length,
    };

    nativeToken = selectedNft?.nativeToken;

    if (
      selectedNft?.nativeToken?.cmcId &&
      Number(selectedNft?.nativeToken?.cmcId) !== 0
    ) {
      priceSubscribe(
        [Number(selectedNft?.nativeToken?.cmcId)],
        selectedNft?.collection?.chain,
      );
    }
  };
...
<ErrorBoundary>
  <div class="flex flex-col justify-between gap-6">
    <div class="flex-1 flex md:flex-row flex-col justify-between gap-6">
      <OverviewCard title={$t("Position Value")}>
  ...
  >
    <div class="flex justify-between items-center">
      <div class="text-2xl font-medium">{$t("List NFT")}</div>
