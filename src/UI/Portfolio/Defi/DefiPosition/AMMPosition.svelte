<script lang="ts">
  ...
  import StonfiWithdraw from "../StonfiWithdraw.svelte";
...
<OverlaySidebar isOpen={showPositionDetail}>
  ...
  {#if showPositionDetail}
    <div class="flex flex-col gap-2">
      {#if ["STON.fi"].includes(protocolInfo?.protocol)}
        <StonfiWithdraw {handleShowStatusModal} {selectedPosition} />
      {/if}

      {#if selectedPosition?.chain !== "TON"}
