<script lang="ts">
  import { onMount } from "svelte";
  import VirtualList from "svelte-tiny-virtual-list";
  import {
    wallet,
    user,
    tab,
    selectedBundle,
    realtimePrice,
    isDarkMode,
    typeWallet,
    showScallopDeFiFarmModal,
    selectedTokenDetail,
  } from "~/store";
  import {
    shorterName,
    shorterAddress,
    wait,
    getChangePercent,
  } from "~/utils/index";
  import {
    calculatePnL,
    calculateRealizedProfit,
    calculateUnrealizedProfit,
    triggerToast,
  } from "~/utils/functions";
  import {
    listSupported,
    detectedChain,
    chainList,
    chainMoveList,
    chainSolList,
  } from "~/lib/chains";
  import numeral from "numeral";
  import { Progressbar } from "flowbite-svelte";
  import dayjs from "dayjs";
  import { nimbus } from "~/lib/network";
  import CopyToClipboard from "svelte-copy-to-clipboard";
  import mixpanel from "mixpanel-browser";
  import tooltip from "~/lib/tooltip";
  import { isNimbusVersion } from "~/utils/constants";
  import { t } from "~/lib/i18n";

  import Tooltip from "~/components/Tooltip.svelte";
  import TooltipNumber from "~/components/TooltipNumber.svelte";
  import Button from "~/components/Button.svelte";
  import Copy from "~/components/Copy.svelte";
  import Image from "~/components/Image.svelte";
  import OverlaySidebar from "~/components/OverlaySidebar.svelte";
  import OverlaySidebarSwap from "~/components/OverlaySidebarSwap.svelte";
  import AppOverlay from "~/components/Overlay.svelte";
  import LoadingPremium from "~/components/LoadingPremium.svelte";
  import Loading from "~/components/Loading.svelte";
  import VaultTable from "~/UI/Portfolio/VaultTable.svelte";
  import TokenDetailSidebar from "~/UI/TokenDetail/TokenDetailSidebar.svelte";
  import Table from "~/components/Table.svelte";
  import ScallopProvideLiquidity from "./Defi/ScallopProvideLiquidity.svelte";
  import Swap from "./Swap.svelte";

  import TrendUp from "~/assets/trend-up.svg";
  import TrendDown from "~/assets/trend-down.svg";
  import defaultToken from "~/assets/defaultToken.png";

  export let checkScamToken;
  export let filterTokenType;
  export let sumTokens;
  export let defaultData;
  export let dataTokenHolding;
  export let isLoading = false;
  export let handleUpdateTotalTokenHolding = (data) => {};

  let data = dataTokenHolding;

  let isShowTooltipName = false;
  let isShowTooltipSymbol = false;
  let isShowCMC = false;
  let isShowCoingecko = false;
  let isShowReport = false;
  let isShowSwap = false;
  let isShowTooltipReport = false;

  let showTableVaults = false;
  let selectedVaults: any;
  let selectedVaultsSymbol: any;

  let isShowReportTable = false;
  let isOldToken = false;
  let isLoadingReportTrashCoin = false;
  let reportReasonList = [];

  let showTokenInfoBundle = false;

  let showSideTokenDetail = false;
  let isCopied = false;
  let isShowTooltipContractAddress = false;

  let showSideTokenSwap = false;

  let virtualList: any;
  let listSelectedIndex: any = [];
  let selectedItemIndex = -1;

  let listTrashToken: any = [];

  const listChainSupportedSwap = [
    // "SOL",
    "TON",
    ...chainMoveList
      .filter((item) => item.value !== "ALL")
      .map((item) => item.value),
    ...chainList
      .filter((item) => item.value !== "ALL")
      .map((item) => item.value),
  ];

  const reasonReport = [
    {
      id: "trash",
      content: "This token is trash 🗑️",
    },
    {
      id: "scam",
      content: "This token is the scam 🤬",
    },
    {
      id: "hate",
      content: "I hate this token 😠",
    },
  ];

  const closeSideBar = (event: any) => {
    if (event.key === "Escape") {
      showSideTokenSwap = false;
      showSideTokenDetail = false;
      selectedTokenDetail.update((n) => (n = {}));
    }
  };

  const handleReportTrashCoin = async () => {
    isLoadingReportTrashCoin = true;
    try {
      let reason = "";

      if (document.getElementById("trash").checked) {
        reason += "Trash Token, ";
      }

      if (document.getElementById("hate").checked) {
        reason += "Hate Token, ";
      }

      if (document.getElementById("scam").checked) {
        reason += "Scam Token, ";
      }

      if (document.getElementById("outdated").checked) {
        reason += document.getElementById("reason").value;
      }

      const formData = {
        chain: document.getElementById("chain").value,
        contractAddress: document.getElementById("contract_address").value,
        reason: reason,
        contractName: $selectedTokenDetail?.name,
        contractTickerSymbol: $selectedTokenDetail?.symbol,
        logoUrl: $selectedTokenDetail?.logo,
        type: "token",
      };
      await nimbus.post("/holding/trash/report", formData);
      isLoadingReportTrashCoin = false;
      triggerToast("We will update after 2 minutes.", "success");

      listTrashToken = [
        ...listTrashToken,
        {
          chain: formData.chain,
          ca: formData.contractAddress,
          symbol: formData.contractTickerSymbol,
        },
      ];
      localStorage.setItem("listTrashToken", JSON.stringify(listTrashToken));
    } catch (error) {
      isLoadingReportTrashCoin = false;
      triggerToast(
        "Something wrong when report token. Please try again!",
        "fail",
      );
      console.error("error:", error);
    } finally {
      isShowReportTable = false;
    }
  };

  const handleCalculateValue = (data: any, price: any) => {
    return Number(data?.amount) * Number(price);
  };

  const handleCalculateRatio = (data: any, price: any) => {
    const value = Number(data?.amount) * Number(price);
    return (value / sumTokens) * 100;
  };

  const handleCalculatePnL = (data: any) => {
    const realizedProfit = calculateRealizedProfit(
      Number(data?.avgCost),
      Number(data?.profit?.realizedProfit),
    );

    const pnl = calculatePnL(
      realizedProfit.profit,
      Number(data?.market_price || 0),
      Number(data?.amount || 0),
      Number(data?.profit?.totalBought || 0),
      Number(data?.profit?.totalSold || 0),
      Number(data?.profit?.totalGain || 0),
      Number(data?.profit?.cost || 0),
    );

    const unrealizedProfit = calculateUnrealizedProfit(
      realizedProfit.profit,
      Number(data?.avgCost),
      Number(pnl),
    );

    return {
      pnl: realizedProfit.profit + unrealizedProfit.profit,
      percentPnL: realizedProfit.percent + unrealizedProfit.percent,
    };
  };

  const handleCheckVault = (data: any) => {
    if (data && data?.vaults && data?.vaults?.length !== 0) {
      return data?.vaults?.reduce(
        (maxObject: any, currentObject: any) => {
          return currentObject.apy > maxObject.apy ? currentObject : maxObject;
        },
        { apy: -Infinity },
      );
    }
    return undefined;
  };

  const handleCheckWithinLast24Hours = (data: any) => {
    return dayjs().diff(dayjs(data?.last_transferred_at), "hour");
  };

  const handleFormatDataBreakdown = (data: any) => {
    return (data?.breakdown || [])
      ?.map((item: any) => {
        const selectedAddress = $selectedBundle?.accounts?.find(
          (account: any) =>
            account?.id?.toLowerCase() === item?.owner?.toLowerCase() ||
            account?.value?.toLowerCase() === item?.owner?.toLowerCase(),
        );
        return {
          ...item,
          logo: selectedAddress?.logo,
          type: selectedAddress?.type,
          label: selectedAddress?.label,
        };
      })
      ?.filter((item: any) => Number(item?.amount) !== 0)
      ?.sort((a: any, b: any) => b?.amount - a?.amount);
  };

  const handleRecomputeHeight = (index: any) => {
    virtualList.recomputeSizes(index);
  };

  $: {
    if (showSideTokenDetail) {
      mixpanel.track("token_detail_page", {
        address: $wallet,
        token_address: $selectedTokenDetail?.contract_address || "",
      });
    }
  }

  $: {
    if (checkScamToken) {
      if (filterTokenType) {
        if (filterTokenType.value === 0) {
          data = dataTokenHolding?.filter((item: any) => !item.is_spam);
        } else {
          data = dataTokenHolding?.filter(
            (item: any) =>
              item?.amount * item.market_price > filterTokenType.value &&
              !item.is_spam,
          );
        }
      } else {
        data = dataTokenHolding?.filter((item: any) => !item.is_spam);
      }
    } else {
      if (filterTokenType) {
        if (filterTokenType.value === 0) {
          data = dataTokenHolding;
        } else {
          data = dataTokenHolding?.filter(
            (item: any) =>
              item?.amount * item.market_price > filterTokenType.value,
          );
        }
      } else {
        data = dataTokenHolding;
      }
    }
  }

  // check market price and update price real-time
  $: {
    if ($realtimePrice) {
      if (!isLoading) {
        if (data && data.length !== 0) {
          data = data
            ?.map((item: any) => {
              const marketPrice = $realtimePrice[
                item?.cmc_id ||
                  item?.contractAddress ||
                  item?.symbol ||
                  item?.price?.symbol
              ]
                ? Number(
                    $realtimePrice[
                      item?.cmc_id ||
                        item?.contractAddress ||
                        item?.symbol ||
                        item?.price?.symbol
                    ]?.price,
                  )
                : Number(item.market_price);

              return {
                ...item,
                market_price: marketPrice,
                value: Number(item?.amount) * Number(marketPrice),
                percent_24h_price_change:
                  item?.last_24h_price && item?.last_24h_price?.price !== 0
                    ? getChangePercent(
                        Number(marketPrice),
                        Number(item?.last_24h_price?.price),
                      )
                    : null,
              };
            })
            ?.sort((a: any, b: any) => {
              if (a.value < b.value) {
                return 1;
              }
              if (a.value > b.value) {
                return -1;
              }
              return 0;
            });

          handleUpdateTotalTokenHolding({
            totalSumTokens: (data || [])?.reduce(
              (prev: any, item: any) => prev + item.value,
              0,
            ),
            totalSumRealizedProfit: (data || [])
              ?.map((item: any) => {
                return {
                  realized_profit: item?.profit?.realizedProfit || 0,
                };
              })
              ?.reduce(
                (prev: any, item: any) => prev + Number(item.realized_profit),
                0,
              ),
            totalSumUnrealizedProfit: (data || [])
              ?.filter(
                (item: any) =>
                  Number(item?.amount) > 0 && Number(item?.avgCost) !== 0,
              )
              ?.map((item: any) => {
                const price = Number(
                  item?.market_price || item?.price?.price || 0,
                );

                const realizedProfit = calculateRealizedProfit(
                  Number(item?.avgCost),
                  Number(item?.profit?.realizedProfit),
                );

                const pnl = calculatePnL(
                  realizedProfit.profit,
                  Number(price),
                  Number(item?.amount || 0),
                  Number(item?.profit?.totalBought || 0),
                  Number(item?.profit?.totalSold || 0),
                  Number(item?.profit?.totalGain || 0),
                  Number(item?.profit?.cost || 0),
                );

                return {
                  unrealized_profit:
                    Number(item?.avgCost) === 0
                      ? 0
                      : Number(pnl) - realizedProfit.profit,
                };
              })
              ?.reduce(
                (prev: any, item: any) => prev + Number(item.unrealized_profit),
                0,
              ),
          });
        } else {
          handleUpdateTotalTokenHolding({
            totalSumTokens: 0,
            totalSumRealizedProfit: 0,
            totalSumUnrealizedProfit: 0,
          });
        }
      }
    }
  }

  $: {
    if ($tab === "nft") {
      handleUpdateTotalTokenHolding({
        totalSumTokens: 0,
        totalSumRealizedProfit: 0,
        totalSumUnrealizedProfit: 0,
      });
    }
  }

  $: {
    if (listTrashToken) {
      data = data.filter(
        (item: any) =>
          !listTrashToken.some(
            (trashItem: any) =>
              trashItem.ca.toLowerCase() === item.contractAddress.toLowerCase(),
          ),
      );
    }
  }

  onMount(() => {
    const listTrashTokenStorage = localStorage.getItem("listTrashToken");
    if (listTrashTokenStorage) {
      listTrashToken = JSON.parse(listTrashTokenStorage);
    }
  });
</script>

<svelte:window on:keydown={closeSideBar} />

<Table>
  <span slot="desktop">
    {#if data && data.length === 0 && !isLoading}
      <div
        class={`bg_f4f5f8 grid ${listSupported.includes($typeWallet) && $typeWallet === "BUNDLE" ? "grid-cols-8" : "grid-cols-7"}`}
      >
        <div class="col-spans-3 pl-3 py-3 rounded-tl-[10px]">
          <div class="text-left text-xs uppercase font-medium">
            {$t("Assets")}
          </div>
        </div>

        <div class="py-3">
          <div class="text-right text-xs uppercase font-medium">
            {$t("Price/24h Change")}
          </div>
        </div>

        <div class="py-3">
          <div class="text-right text-xs uppercase font-medium">
            {$t("Amount")}
          </div>
        </div>

        <div class="py-3">
          <div class="text-right text-xs uppercase font-medium">
            {$t("Value")}
          </div>
        </div>

        <div class="py-3 pr-3">
          <div class="text-right text-xs uppercase font-medium">PnL</div>
        </div>

        {#if listSupported.includes($typeWallet) && $typeWallet === "BUNDLE"}
          <div class="py-3 rounded-tr-[10px]" />
        {/if}
      </div>
      <div
        class={`grid ${listSupported.includes($typeWallet) && $typeWallet === "BUNDLE" ? "grid-cols-8" : "grid-cols-7"}`}
      >
        <div
          class="col-span-full flex justify-center items-center h-[465px] py-3 px-3 text-base text-gray-400"
        >
          {#if defaultData && defaultData.length === 0}
            {$t("Empty")}
          {:else}
            {$t("All tokens less than")} ${filterTokenType.value}
          {/if}
        </div>
      </div>
    {:else}
      <VirtualList
        scrollDirection="vertical"
        width="100%"
        height={data.length < 30 ? data.length * 75 + 1 + 40 : 2250}
        bind:this={virtualList}
        itemCount={data.length + 1}
        itemSize={(index) => {
          if (index > 0) {
            const formatDataBreakdown = handleFormatDataBreakdown(
              data[selectedItemIndex],
            );
            return listSelectedIndex.includes(index - 1)
              ? formatDataBreakdown.length > 12
                ? 500 + 135
                : formatDataBreakdown.length * 75 + 135
              : 75;
          } else {
            return 40;
          }
        }}
        stickyIndices={[0]}
      >
        <div
          class={`grid group transition-all ${index > 0 ? "cursor-pointer" : ""} ${listSupported.includes($typeWallet) && $typeWallet === "BUNDLE" ? "grid-cols-8" : "grid-cols-7"}`}
          slot="item"
          let:index
          let:style
          {style}
          on:mouseover={() => {
            if (index > 0) {
              selectedItemIndex = index - 1;
              isShowSwap = true;
              if ($user && Object.keys($user).length !== 0) {
                isShowReport = true;
              }
              if (data[index - 1]?.cmc_slug) {
                isShowCMC = true;
              }
              if (data[index - 1]?.cg_id) {
                isShowCoingecko = true;
              }
            }
          }}
          on:mouseleave={() => {
            if (index > 0) {
              selectedItemIndex = -1;
              isShowSwap = false;
              if ($user && Object.keys($user).length !== 0) {
                isShowReport = false;
              }
              if (data[index - 1]?.cmc_slug) {
                isShowCMC = false;
              }
              if (data[index - 1]?.cg_id) {
                isShowCoingecko = false;
              }
            }
          }}
          on:click={() => {
            if (index > 0) {
              if (listSupported.includes($typeWallet)) {
                showSideTokenDetail = true;
                selectedTokenDetail.update((n) => (n = data[index - 1]));
              }
            }
          }}
        >
          {#if index === 0}
            <div class="bg_f4f5f8 col-spans-3 pl-3 py-3 rounded-tl-[10px]">
              <div class="text-left text-xs uppercase font-medium">
                {$t("Assets")}
              </div>
            </div>

            <div class="bg_f4f5f8 py-3">
              <div class="text-right text-xs uppercase font-medium">
                {$t("Price/24h Change")}
              </div>
            </div>

            <div class="bg_f4f5f8 py-3">
              <div class="text-right text-xs uppercase font-medium">
                {$t("Amount")}
              </div>
            </div>

            <div class="bg_f4f5f8 py-3">
              <div class="text-right text-xs uppercase font-medium">
                {$t("Value")}
              </div>
            </div>

            <div class="bg_f4f5f8 py-3 pr-3">
              <div class="text-right text-xs uppercase font-medium">PnL</div>
            </div>

            {#if listSupported.includes($typeWallet) && $typeWallet === "BUNDLE"}
              <div class="bg_f4f5f8 py-3 rounded-tr-[10px]" />
            {/if}
          {:else}
            <div
              class={`col-spans-3 pl-3 py-3 ${
                listSelectedIndex.includes(index - 1)
                  ? $isDarkMode
                    ? "bg-[#000]"
                    : "bg-gray-100"
                  : $isDarkMode
                    ? "bg-[#131313] group-hover:bg-[#000]"
                    : "bg-white group-hover:bg-gray-100"
              }`}
              style={`${data.length - 1 === index - 1 ? "border-bottom-left-radius: 10px;" : ""}`}
            >
              <div class="relative flex items-center justify-start gap-3">
                <div class="relative">
                  <div class="rounded-full w-[30px] h-[30px] overflow-hidden">
                    <Image
                      logo={data[index - 1]?.logo}
                      defaultLogo={defaultToken}
                    />
                  </div>
                  {#if ["EVM", "MOVE", "SOL", "BUNDLE"].includes($typeWallet)}
                    <div class="absolute -top-2 -right-1">
                      <img
                        src={detectedChain(data[index - 1]?.chain)?.logo}
                        alt=""
                        width="15"
                        height="15"
                        class="rounded-full"
                      />
                    </div>
                  {/if}
                </div>

                <div class="flex flex-col gap-1">
                  <div class="flex items-start gap-2">
                    <div
                      class="relative font-medium text-sm"
                      on:mouseover={() => {
                        if (index > 0) {
                          selectedItemIndex = index - 1;
                          isShowTooltipName = true;
                        }
                      }}
                      on:mouseleave={() => {
                        if (index > 0) {
                          selectedItemIndex = -1;
                          isShowTooltipName = false;
                        }
                      }}
                    >
                      {#if !data[index - 1]?.name || data[index - 1]?.name === "N/A"}
                        <span>
                          {shorterAddress(data[index - 1]?.contractAddress)}
                        </span>
                      {:else}
                        <span>
                          {#if data[index - 1]?.name?.length > 34}
                            {shorterName(data[index - 1]?.name, 34)}
                          {:else}
                            {data[index - 1]?.name}
                          {/if}
                        </span>
                      {/if}

                      {#if isShowTooltipName && selectedItemIndex === index - 1 && data[index - 1]?.name?.length > 12}
                        <div
                          class="absolute left-0 -top-8"
                          style="z-index: 2147483648;"
                        >
                          <Tooltip text={data[index - 1].name} />
                        </div>
                      {/if}
                    </div>

                    <!-- {#if handleCheckVault(data[index - 1]) !== undefined && isNimbusVersion}
                      <div
                        class="w-max flex items-center justyfy-center px-2 py-1 text_27326F text-[10px] font-medium bg-[#1e96fc33] rounded-[1000px] cursor-pointer"
                        on:click={(e) => {
                          if (index > 0) {
                            e.stopPropagation();
                            showTableVaults = true;
                            selectedVaults = data[index - 1].vaults;
                            selectedVaultsSymbol = data[index - 1].symbol;
                            selectedTokenDetail.update(
                              (n) => (n = data[index - 1]),
                            );
                          }
                        }}
                      >
                        <div class="flex gap-1">
                          Farm up to
                          {numeral(
                            handleCheckVault(data[index - 1])?.apy * 100,
                          ).format("0,0.00")}% APY
                        </div>
                      </div>
                    {/if} -->
                  </div>

                  <div class="flex items-center gap-2 min-h-[20px]">
                    <div
                      class="relative font-medium text_00000080 text-xs"
                      on:mouseover={() => {
                        if (index > 0) {
                          selectedItemIndex = index - 1;
                          isShowTooltipSymbol = true;
                        }
                      }}
                      on:mouseleave={() => {
                        if (index > 0) {
                          selectedItemIndex = -1;
                          isShowTooltipSymbol = false;
                        }
                      }}
                    >
                      {#if data[index - 1]?.symbol === undefined}
                        N/A
                      {:else}
                        {shorterName(data[index - 1]?.symbol, 12)}
                      {/if}
                      {#if isShowTooltipSymbol && selectedItemIndex === index - 1 && data[index - 1].symbol.length > 12}
                        <div
                          class="absolute left-0 -top-8"
                          style="z-index: 2147483648;"
                        >
                          <Tooltip text={data[index - 1]?.symbol} />
                        </div>
                      {/if}
                    </div>

                    {#if data[index - 1]?.positionType === "ERC_404"}
                      <span
                        class="inline-flex items-center gap-x-1.5 rounded-full bg-yellow-100 px-1 py-0.5 text-[10px] font-medium text-yellow-800"
                      >
                        <svg
                          class="h-1.5 w-1.5 fill-yellow-500"
                          viewBox="0 0 6 6"
                          aria-hidden="true"
                        >
                          <circle cx={3} cy={3} r={3} />
                        </svg>
                        ERC 404
                      </span>
                    {/if}

                    {#if data[index - 1]?.is_spam}
                      <div
                        class="text-[10px] rounded-[30px] px-2 py-1 w-max mt-1"
                        style="background-color: #FF5733; color: #FFD3B6;"
                      >
                        SCAM
                      </div>
                    {/if}

                    {#if isShowReport && selectedItemIndex === index - 1}
                      <div
                        class="relative w-5 cursor-pointer"
                        on:click={(e) => {
                          if (index > 0) {
                            e.stopPropagation();
                            isShowReportTable = true;
                            selectedTokenDetail.update(
                              (n) => (n = data[index - 1]),
                            );
                          }
                        }}
                        on:mouseover={() => {
                          if (index > 0) {
                            selectedItemIndex = index - 1;
                            isShowTooltipReport = true;
                          }
                        }}
                        on:mouseleave={() => {
                          if (index > 0) {
                            selectedItemIndex = -1;
                            isShowTooltipReport = false;
                          }
                        }}
                      >
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          width="16"
                          height="16"
                          viewBox="0 0 24 24"
                        >
                          <g
                            fill="none"
                            stroke-linecap="round"
                            stroke-linejoin="round"
                            stroke-width="2"
                          >
                            <path d="M0 0h24v24H0z" />
                            <path
                              fill="currentColor"
                              d="M19 4c.852 0 1.297.986.783 1.623l-.076.084L15.915 9.5l3.792 3.793c.603.602.22 1.614-.593 1.701L19 15H6v6a1 1 0 0 1-.883.993L5 22a1 1 0 0 1-.993-.883L4 21V5a1 1 0 0 1 .883-.993L5 4h14z"
                            />
                          </g>
                        </svg>
                        {#if isShowTooltipReport && selectedItemIndex === index - 1}
                          <div
                            class="absolute -top-8 left-1/2 transform -translate-x-1/2"
                            style="z-index: 2147483648;"
                          >
                            <Tooltip text="Report" />
                          </div>
                        {/if}
                      </div>
                    {/if}

                    {#if isShowCMC && selectedItemIndex === index - 1}
                      <a
                        href={`https://coinmarketcap.com/currencies/${data[index - 1]?.cmc_slug}`}
                        on:click={(e) => {
                          if (index > 0) {
                            e.stopPropagation();
                          }
                        }}
                        target="_blank"
                        class="w-[20px] h-[20px] cursor-pointer"
                      >
                        <svg
                          viewBox="0 0 76.52 77.67"
                          xmlns="http://www.w3.org/2000/svg"
                          class="object-contain w-full h-full rounded-full"
                        >
                          <path
                            d="m66.54 46.41a4.09 4.09 0 0 1 -4.17.28c-1.54-.87-2.37-2.91-2.37-5.69v-8.52c0-4.09-1.62-7-4.33-7.79-4.58-1.34-8 4.27-9.32 6.38l-8.1 13.11v-16c-.09-3.69-1.29-5.9-3.56-6.56-1.5-.44-3.75-.26-5.94 3.08l-18.11 29.07a32 32 0 0 1 -3.64-14.94c0-17.52 14-31.77 31.25-31.77s31.3 14.25 31.3 31.77v.09s0 .06 0 .09c.17 3.39-.93 6.09-3 7.4zm10-7.57v-.17c-.14-21.35-17.26-38.67-38.29-38.67s-38.25 17.42-38.25 38.83 17.16 38.84 38.25 38.84a37.81 37.81 0 0 0 26-10.36 3.56 3.56 0 0 0 .18-5 3.43 3.43 0 0 0 -4.86-.23 30.93 30.93 0 0 1 -44.57-2.08l16.3-26.2v12.09c0 5.81 2.25 7.69 4.14 8.24s4.78.17 7.81-4.75l9-14.57c.28-.47.55-.87.79-1.22v7.41c0 5.43 2.18 9.77 6 11.91a11 11 0 0 0 11.21-.45c4.2-2.73 6.49-7.67 6.25-13.62z"
                            fill={`${$isDarkMode ? "#d1d5db" : "#17181b"}`}
                          />
                        </svg>
                      </a>
                    {/if}

                    {#if isShowCoingecko && selectedItemIndex === index - 1}
                      <a
                        href={`https://www.coingecko.com/en/coins/${data[index - 1]?.cg_id}`}
                        on:click={(e) => {
                          if (index > 0) {
                            e.stopPropagation();
                          }
                        }}
                        target="_blank"
                        class="w-[20px] h-[20px] cursor-pointer"
                      >
                        <svg
                          version="1.1"
                          id="Layer_1"
                          xmlns="http://www.w3.org/2000/svg"
                          xmlns:xlink="http://www.w3.org/1999/xlink"
                          x="0px"
                          y="0px"
                          class="object-contain w-full h-full rounded-full"
                          viewBox="0 0 130 130"
                          enable-background="new 0 0 130 130"
                          xml:space="preserve"
                        >
                          <path
                            fill="#8CC640"
                            opacity="1.000000"
                            stroke="none"
                            d="
                    M131.000000,57.000000 
                      C131.000000,63.021152 131.000000,69.042305 130.672409,75.726242 
                      C126.245422,96.442734 115.971336,112.377365 97.906433,122.312782 
                      C90.821442,126.209435 82.667252,128.162033 75.000000,131.000000 
                      C68.978851,131.000000 62.957695,131.000000 56.274689,130.671448 
                      C26.738447,123.925354 8.627264,106.579208 1.978957,77.565338 
                      C1.849081,76.998550 1.334888,76.519814 1.000000,76.000000 
                      C1.000000,69.645599 1.000000,63.291203 1.327633,56.273994 
                      C7.883286,27.381535 24.726683,9.368647 52.947453,2.288438 
                      C54.319679,1.944165 55.650108,1.433293 57.000000,1.000000 
                      C62.688042,1.000000 68.376083,1.000000 74.741592,1.327464 
                      C96.446571,5.317031 112.630692,16.197533 122.822418,34.953358 
                      C126.526794,41.770519 128.329590,49.620987 131.000000,57.000000 
                    M67.669617,34.593678 
                      C72.853539,36.328110 78.037468,38.062542 83.988960,39.800339 
                      C91.320595,42.477032 98.943100,44.560196 105.912613,47.975792 
                      C115.309883,52.581173 116.758698,60.664780 110.240662,68.973640 
                      C107.172386,72.884933 103.662323,76.491829 100.067467,79.939499 
                      C91.340828,88.308846 83.506844,97.323128 78.944870,108.687225 
                      C76.923164,113.723396 75.289993,118.956802 76.719704,124.519165 
                      C99.219704,123.777618 121.628136,100.428284 125.708771,73.910149 
                      C129.903824,46.648495 112.813217,18.713875 86.159439,9.266678 
                      C58.672554,-0.475806 28.585316,10.712248 13.686728,36.168118 
                      C0.647401,58.447243 5.632729,87.567360 16.799000,99.079071 
                      C21.500576,87.592857 23.501854,75.702583 23.929556,63.478218 
                      C24.379354,50.622173 30.065470,41.486504 42.244755,36.285854 
                      C50.463642,32.776325 58.658878,32.550331 67.669617,34.593678 
                    M46.378696,47.004932 
                      C46.153244,48.472221 45.722897,49.941711 45.738663,51.406406 
                      C45.799110,57.022385 52.580402,64.181587 57.331547,63.782600 
                      C64.207664,63.205166 69.441216,57.424793 69.158218,50.720249 
                      C68.941345,45.582127 65.791763,41.742584 60.648354,40.346165 
                      C54.984890,38.808552 50.460312,40.761738 46.378696,47.004932 
                    M70.502975,77.108353 
                      C70.182304,77.152718 69.855530,77.173737 69.544479,77.254951 
                      C69.444435,77.281075 69.385818,77.465889 69.308258,77.578117 
                      C70.492355,78.370712 71.577995,79.583702 72.876495,79.887817 
                      C81.442947,81.894165 89.345863,79.361977 96.649796,76.835396 
                      C88.453735,76.963951 79.822350,77.099335 70.502975,77.108353 
                    z"
                          />
                          <path
                            fill="#FEFEFD"
                            opacity="1.000000"
                            stroke="none"
                            d="
                    M131.000000,56.531342 
                      C128.329590,49.620987 126.526794,41.770519 122.822418,34.953358 
                      C112.630692,16.197533 96.446571,5.317031 75.209534,1.327464 
                      C93.593452,1.000000 112.186913,1.000000 131.000000,1.000000 
                      C131.000000,19.353643 131.000000,37.708164 131.000000,56.531342 
                    z"
                          />
                          <path
                            fill="#FEFEFD"
                            opacity="1.000000"
                            stroke="none"
                            d="
                    M56.531342,1.000000 
                      C55.650108,1.433293 54.319679,1.944165 52.947453,2.288438 
                      C24.726683,9.368647 7.883286,27.381535 1.327633,55.805592 
                      C1.000000,37.739223 1.000000,19.478447 1.000000,1.000000 
                      C19.353655,1.000000 37.708168,1.000000 56.531342,1.000000 
                    z"
                          />
                          <path
                            fill="#FEFEFD"
                            opacity="1.000000"
                            stroke="none"
                            d="
                    M75.468658,131.000000 
                      C82.667252,128.162033 90.821442,126.209435 97.906433,122.312782 
                      C115.971336,112.377365 126.245422,96.442734 130.672409,76.194504 
                      C131.000000,94.260780 131.000000,112.521553 131.000000,131.000000 
                      C112.646347,131.000000 94.291832,131.000000 75.468658,131.000000 
                    z"
                          />
                          <path
                            fill="#FEFEFD"
                            opacity="1.000000"
                            stroke="none"
                            d="
                    M1.000000,76.468658 
                      C1.334888,76.519814 1.849081,76.998550 1.978957,77.565338 
                      C8.627264,106.579208 26.738447,123.925354 55.806419,130.671448 
                      C37.739883,131.000000 19.479769,131.000000 1.000000,131.000000 
                      C1.000000,112.979698 1.000000,94.958511 1.000000,76.468658 
                    z"
                          />
                          <path
                            fill="#F8E987"
                            opacity="1.000000"
                            stroke="none"
                            d="
                    M67.161728,34.130535 
                      C58.658878,32.550331 50.463642,32.776325 42.244755,36.285854 
                      C30.065470,41.486504 24.379354,50.622173 23.929556,63.478218 
                      C23.501854,75.702583 21.500576,87.592857 16.799000,99.079071 
                      C5.632729,87.567360 0.647401,58.447243 13.686728,36.168118 
                      C28.585316,10.712248 58.672554,-0.475806 86.159439,9.266678 
                      C112.813217,18.713875 129.903824,46.648495 125.708771,73.910149 
                      C121.628136,100.428284 99.219704,123.777618 76.719704,124.519165 
                      C75.289993,118.956802 76.923164,113.723396 78.944870,108.687225 
                      C83.506844,97.323128 91.340828,88.308846 100.067467,79.939499 
                      C103.662323,76.491829 107.172386,72.884933 110.240662,68.973640 
                      C116.758698,60.664780 115.309883,52.581173 105.912613,47.975792 
                      C98.943100,44.560196 91.320595,42.477032 83.481522,39.486412 
                      C79.000679,33.218479 75.050224,31.958828 67.161728,34.130535 
                    M122.741554,62.952946 
                      C121.931564,49.456993 117.821663,37.472195 107.482773,27.581638 
                      C112.618309,40.665421 117.481361,53.054993 122.356270,65.474770 
                      C122.471214,64.915009 122.599434,64.290619 122.741554,62.952946 
                    M74.766914,11.836439 
                      C80.884399,13.444856 87.001877,15.053272 93.647499,16.800549 
                      C88.945778,11.339518 75.633400,7.880075 68.937065,10.319439 
                      C70.920326,10.843212 72.457603,11.249201 74.766914,11.836439 
                    z"
                          />
                          <path
                            fill="#F5F7F2"
                            opacity="1.000000"
                            stroke="none"
                            d="
                    M46.557922,46.678169 
                      C50.460312,40.761738 54.984890,38.808552 60.648354,40.346165 
                      C65.791763,41.742584 68.941345,45.582127 69.158218,50.720249 
                      C69.441216,57.424793 64.207664,63.205166 57.331547,63.782600 
                      C52.580402,64.181587 45.799110,57.022385 45.738663,51.406406 
                      C45.722897,49.941711 46.153244,48.472221 46.557922,46.678169 
                    M59.129066,43.321339 
                      C52.905285,43.682343 50.128323,45.922436 49.880501,50.781902 
                      C49.652809,55.246601 51.976357,58.415615 56.005619,59.135765 
                      C60.652447,59.966290 64.273163,58.019295 65.559769,53.998142 
                      C67.025444,49.417343 65.020790,45.732014 59.129066,43.321339 
                    z"
                          />
                          <path
                            fill="#5E6658"
                            opacity="1.000000"
                            stroke="none"
                            d="
                    M70.846970,77.171539 
                      C79.822350,77.099335 88.453735,76.963951 96.649796,76.835396 
                      C89.345863,79.361977 81.442947,81.894165 72.876495,79.887817 
                      C71.577995,79.583702 70.492355,78.370712 69.308258,77.578117 
                      C69.385818,77.465889 69.444435,77.281075 69.544479,77.254951 
                      C69.855530,77.173737 70.182304,77.152718 70.846970,77.171539 
                    z"
                          />
                          <path
                            fill="#249B48"
                            opacity="1.000000"
                            stroke="none"
                            d="
                    M67.415672,34.362106 
                      C75.050224,31.958828 79.000679,33.218479 83.097733,39.484730 
                      C78.037468,38.062542 72.853539,36.328110 67.415672,34.362106 
                    z"
                          />
                          <path
                            fill="#FDFDF4"
                            opacity="1.000000"
                            stroke="none"
                            d="
                    M122.734604,63.309586 
                      C122.599434,64.290619 122.471214,64.915009 122.356270,65.474770 
                      C117.481361,53.054993 112.618309,40.665421 107.482773,27.581638 
                      C117.821663,37.472195 121.931564,49.456993 122.734604,63.309586 
                    z"
                          />
                          <path
                            fill="#FCFBEE"
                            opacity="1.000000"
                            stroke="none"
                            d="
                    M74.380898,11.745815 
                      C72.457603,11.249201 70.920326,10.843212 68.937065,10.319439 
                      C75.633400,7.880075 88.945778,11.339518 93.647499,16.800549 
                      C87.001877,15.053272 80.884399,13.444856 74.380898,11.745815 
                    z"
                          />
                          <path
                            fill="#5B5C5E"
                            opacity="1.000000"
                            stroke="none"
                            d="
                    M59.497826,43.426590 
                      C65.020790,45.732014 67.025444,49.417343 65.559769,53.998142 
                      C64.273163,58.019295 60.652447,59.966290 56.005619,59.135765 
                      C51.976357,58.415615 49.652809,55.246601 49.880501,50.781902 
                      C50.128323,45.922436 52.905285,43.682343 59.497826,43.426590 
                    z"
                          />
                        </svg>
                      </a>
                    {/if}
                  </div>
                </div>

                {#if isShowSwap && selectedItemIndex === index - 1 && listChainSupportedSwap.includes(data[index - 1]?.chain)}
                  <div class="w-max">
                    <Button
                      variant="tertiary"
                      on:click={(e) => {
                        if (index > 0) {
                          e.stopPropagation();
                          showSideTokenSwap = true;
                          selectedTokenDetail.update(
                            (n) => (n = data[index - 1]),
                          );
                        }
                      }}
                    >
                      <div class="text-white text-[11px]">Swap</div>
                    </Button>
                  </div>
                {/if}
              </div>
            </div>

            <div
              class={`py-3 ${
                listSelectedIndex.includes(index - 1)
                  ? $isDarkMode
                    ? "bg-[#000]"
                    : "bg-gray-100"
                  : $isDarkMode
                    ? "bg-[#131313] group-hover:bg-[#000]"
                    : "bg-white group-hover:bg-gray-100"
              }`}
            >
              <div class="h-[50px]">
                <div
                  class="flex items-center justify-end font-medium text-sm text_00000099"
                >
                  $<TooltipNumber
                    number={Number(data[index - 1].market_price)}
                    type="balance"
                  />
                </div>

                <div
                  class="flex items-center justify-end font-medium text-sm gap-1"
                >
                  {#if data[index - 1].percent_24h_price_change === null || data[index - 1].percent_24h_price_change === 0}
                    <div class="text_00000099">-</div>
                  {:else}
                    <div
                      class={`${
                        data[index - 1].percent_24h_price_change >= 0
                          ? "text-[#00A878]"
                          : "text-red-500"
                      }`}
                    >
                      <TooltipNumber
                        number={Math.abs(
                          data[index - 1].percent_24h_price_change,
                        )}
                        type={Math.abs(
                          data[index - 1].percent_24h_price_change,
                        ) > 999999 ||
                        Math.abs(data[index - 1].percent_24h_price_change) <
                          0.001
                          ? "balance"
                          : "percent"}
                      />
                      <span>%</span>
                    </div>
                    <img
                      src={data[index - 1].percent_24h_price_change !== 0
                        ? data[index - 1].percent_24h_price_change >= 0
                          ? TrendUp
                          : TrendDown
                        : ""}
                      alt="trend"
                      class="mb-1"
                    />
                  {/if}
                </div>
              </div>
            </div>

            <div
              class={`py-3 ${
                listSelectedIndex.includes(index - 1)
                  ? $isDarkMode
                    ? "bg-[#000]"
                    : "bg-gray-100"
                  : $isDarkMode
                    ? "bg-[#131313] group-hover:bg-[#000]"
                    : "bg-white group-hover:bg-gray-100"
              }`}
            >
              <div
                class="h-[50px] flex items-center justify-end gap-1 font-medium text-sm text_00000099"
              >
                {#if handleCheckWithinLast24Hours(data[index - 1]) < 24 && handleCheckWithinLast24Hours(data[index - 1]) > 0}
                  <span
                    use:tooltip={{
                      content: `<tooltip-detail text="Changed ${handleCheckWithinLast24Hours(data[index - 1])} hrs ago" />`,
                      allowHTML: true,
                      placement: "top",
                      interactive: true,
                    }}
                    class="cursor-pointer"
                  >
                    <div class="w-2 h-2 bg-indigo-500 rounded-full" />
                  </span>
                {/if}
                <TooltipNumber
                  number={data[index - 1]?.amount || 0}
                  type="balance"
                  personalValue
                />
              </div>
            </div>

            <div
              class={`py-3 ${
                listSelectedIndex.includes(index - 1)
                  ? $isDarkMode
                    ? "bg-[#000]"
                    : "bg-gray-100"
                  : $isDarkMode
                    ? "bg-[#131313] group-hover:bg-[#000]"
                    : "bg-white group-hover:bg-gray-100"
              }`}
            >
              <div class="flex justify-center h-[50px] flex-col gap-1">
                <div class="flex justify-end font-medium text-sm text_00000099">
                  <TooltipNumber
                    number={handleCalculateValue(
                      data[index - 1],
                      Number(data[index - 1].market_price),
                    )}
                    type="value"
                    personalValue
                  />
                </div>
                <!-- <div class="flex flex-col items-end justify-end gap-1">
                  <div class="flex justify-end text-gray-400 text-sm">
                    <TooltipNumber
                      number={handleCalculateRatio(
                        data[index - 1],
                        Number(data[index - 1].market_price),
                      )}
                      type="percent"
                    />%
                  </div>

                  <div class="w-3/4 max-w-20">
                    <Progressbar
                      progress={handleCalculateRatio(
                        data[index - 1],
                        Number(data[index - 1].market_price),
                      )}
                      animate
                      size="h-1"
                    />
                  </div>
                </div> -->
              </div>
            </div>

            <div
              class={`py-3 pr-3 ${
                listSelectedIndex.includes(index - 1)
                  ? $isDarkMode
                    ? "bg-[#000]"
                    : "bg-gray-100"
                  : $isDarkMode
                    ? "bg-[#131313] group-hover:bg-[#000]"
                    : "bg-white group-hover:bg-gray-100"
              }`}
            >
              <div
                class="view-pnl flex items-center justify-end gap-1 font-medium text-sm"
              >
                {#if ["BTC"].includes($typeWallet)}
                  N/A
                {:else}
                  <div class="flex flex-col">
                    <div
                      class={`flex justify-end ${
                        handleCalculatePnL(data[index - 1])?.pnl !== 0
                          ? handleCalculatePnL(data[index - 1])?.pnl >= 0
                            ? "text-[#00A878]"
                            : "text-red-500"
                          : "text_00000099"
                      }`}
                    >
                      <TooltipNumber
                        number={Math.abs(
                          handleCalculatePnL(data[index - 1])?.pnl,
                        )}
                        type="value"
                        personalValue
                      />
                    </div>
                    <div class="flex items-center justify-end gap-1">
                      <div
                        class={`flex items-center ${
                          handleCalculatePnL(data[index - 1])?.pnl !== 0
                            ? handleCalculatePnL(data[index - 1])?.pnl >= 0
                              ? "text-[#00A878]"
                              : "text-red-500"
                            : "text_00000099"
                        }`}
                      >
                        <TooltipNumber
                          number={Math.abs(
                            handleCalculatePnL(data[index - 1])?.percentPnL,
                          ) * 100}
                          type={Math.abs(
                            handleCalculatePnL(data[index - 1])?.percentPnL,
                          ) *
                            100 >
                          999999
                            ? "balance"
                            : "percent"}
                        />
                        <span>%</span>
                      </div>

                      {#if handleCalculatePnL(data[index - 1])?.pnl !== 0}
                        <img
                          src={handleCalculatePnL(data[index - 1])?.pnl !== 0
                            ? handleCalculatePnL(data[index - 1])?.pnl >= 0
                              ? TrendUp
                              : TrendDown
                            : ""}
                          alt="trend"
                          class="mb-1"
                        />
                      {/if}
                    </div>
                  </div>
                {/if}
              </div>
            </div>

            {#if listSupported.includes($typeWallet) && $typeWallet === "BUNDLE"}
              <div
                class={`py-3 w-full ${
                  listSelectedIndex.includes(index - 1)
                    ? $isDarkMode
                      ? "bg-[#000]"
                      : "bg-gray-100"
                    : $isDarkMode
                      ? "bg-[#131313] group-hover:bg-[#000]"
                      : "bg-white group-hover:bg-gray-100"
                }`}
                style={`${data.length - 1 === index - 1 ? "border-bottom-right-radius: 10px;" : ""}`}
              >
                <div
                  class="2xl:pl-14 pl-4 w-full h-[50px] flex items-center justify-center 2xl:gap-7 gap-6"
                >
                  <div
                    class="flex justify-center"
                    use:tooltip={{
                      content: `<tooltip-detail text="Show bundles detail" />`,
                      allowHTML: true,
                      placement: "top",
                    }}
                  >
                    <div
                      class="cursor-pointer transform rotate-180 xl:w-3 xl:h-3 w-5 h-5"
                      class:rotate-0={listSelectedIndex.includes(index - 1)}
                      on:click={(e) => {
                        if (index > 0) {
                          e.stopPropagation();
                          listSelectedIndex.includes(index - 1)
                            ? (listSelectedIndex = listSelectedIndex.filter(
                                (item) => item !== index - 1,
                              ))
                            : (listSelectedIndex = [
                                ...listSelectedIndex,
                                index - 1,
                              ]);
                          selectedItemIndex = index - 1;
                          handleRecomputeHeight(index - 1);
                        }
                      }}
                    >
                      <svg
                        width="12"
                        height="12"
                        viewBox="0 0 12 12"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <path
                          d="M10 8.36365L6 4.00001L2 8.36365"
                          stroke={$isDarkMode ? "white" : "#00000080"}
                          stroke-width="2"
                          stroke-linecap="round"
                          stroke-linejoin="round"
                        />
                      </svg>
                    </div>
                  </div>
                </div>
              </div>
            {/if}

            {#if listSelectedIndex.includes(index - 1)}
              <div
                class={`col-span-full border-t-[1px] border_0000000d xl:pt-2 py-2 pl-3 ${$isDarkMode ? "bg-[#000]" : "bg-gray-100"}`}
              >
                <div class="text-sm">
                  {$t("Token breakdown")}
                </div>
              </div>

              <div
                class={`col-span-full ${$isDarkMode ? "bg-[#000]" : "bg-gray-100"}`}
              >
                <div class="-mt-1 flex items-center">
                  <div class="py-2 pl-3 flex-1">
                    <div class="text-left text-sm font-medium">
                      {$t("Account")}
                    </div>
                  </div>
                  <div class="py-2 flex-1">
                    <div class="text-right text-sm font-medium">
                      {$t("Amount")}
                    </div>
                  </div>
                  <div class="py-2 2xl:pr-14 pr-6 flex-1">
                    <div class="text-right text-sm font-medium">
                      {$t("Value")}
                    </div>
                  </div>
                </div>
              </div>

              <div
                class={`col-span-full -mt-2 -mx-[1px] max-h-[500px] overflow-y-auto overflow-x-hidden ${
                  $isDarkMode ? "bg-[#000]" : "bg-gray-100"
                }`}
              >
                {#each handleFormatDataBreakdown(data[index - 1]) as item}
                  <div class="flex items-center">
                    <div class="py-2 pl-3 flex-1">
                      <div class="flex items-center gap-3">
                        <div
                          class="rounded-full w-[30px] h-[30px] overflow-hidden"
                        >
                          <Image
                            logo={data[index - 1].logo}
                            defaultLogo={defaultToken}
                          />
                        </div>
                        <div class="flex flex-col items-start gap-[2px]">
                          <div class="font-medium text-sm text_00000099">
                            {#if $selectedBundle?.label !== $wallet}
                              {item.label}
                            {:else}
                              <div
                                class="flex items-center gap-1"
                                on:click={(e) => e.stopPropagation()}
                              >
                                Owner <Copy
                                  address={item?.label}
                                  iconColor={$isDarkMode ? "#fff" : "#000"}
                                  color={$isDarkMode ? "#fff" : "#000"}
                                  isShorten
                                />
                              </div>
                            {/if}
                          </div>
                          <div
                            class="text-sm flex justify-end items-center"
                            on:click={(e) => e.stopPropagation()}
                          >
                            <Copy
                              address={item?.owner}
                              iconColor={$isDarkMode ? "#fff" : "#000"}
                              color={$isDarkMode ? "#fff" : "#000"}
                              isShorten
                            />
                          </div>
                        </div>
                      </div>
                    </div>

                    <div class="py-2 flex-1">
                      <div
                        class="text-sm text_00000099 flex flex-col justify-end items-end gap-1"
                      >
                        <div
                          class="flex justify-end items-center gap-1 font-medium text-sm text_00000099"
                        >
                          <TooltipNumber
                            number={item?.amount || 0}
                            type="balance"
                          />
                          {#if data[index - 1].symbol === undefined}
                            N/A
                          {:else}
                            {data[index - 1].symbol}
                          {/if}
                        </div>

                        <div
                          class="flex flex-col items-end justify-end gap-1 w-full"
                        >
                          <div class="flex justify-end text-gray-400 text-sm">
                            <TooltipNumber
                              number={Math.abs(
                                item?.amount / data[index - 1]?.amount,
                              ) * 100}
                              type={Math.abs(
                                item?.amount / data[index - 1]?.amount,
                              ) *
                                100 >
                              999999
                                ? "balance"
                                : "percent"}
                            />%
                          </div>
                          <div class="w-3/4 max-w-20">
                            <Progressbar
                              progress={Math.abs(
                                item?.amount / data[index - 1]?.amount,
                              ) * 100}
                              animate
                              size="h-1"
                            />
                          </div>
                        </div>
                      </div>
                    </div>

                    <div class="py-2 2xl:pr-14 pr-6 flex-1">
                      <div
                        class="text-sm text_00000099 font-medium flex justify-end items-center"
                      >
                        $<TooltipNumber
                          number={Number(item?.amount) *
                            Number(data[index - 1]?.market_price)}
                          type="balance"
                        />
                      </div>
                    </div>
                  </div>
                {/each}
              </div>
            {/if}
          {/if}
        </div>
      </VirtualList>
    {/if}

    {#if isLoading}
      <div
        class={`w-full h-full grid ${listSupported.includes($typeWallet) && $typeWallet === "BUNDLE" ? "grid-cols-10" : "grid-cols-9"} ${
          $isDarkMode ? "bg-[#131313]" : "bg-white"
        }`}
      >
        <div
          class="col-span-full flex justify-center items-center h-full py-3 px-3"
        >
          {#if isNimbusVersion}
            <LoadingPremium />
          {:else}
            <Loading />
          {/if}
        </div>
      </div>
    {/if}
  </span>
  <span slot="mobile">
    {#if data && data.length === 0 && !isLoading}
      <div
        class="flex justify-center items-center h-full py-3 px-3 text-base text-gray-400"
      >
        {#if defaultData && defaultData.length === 0}
          {$t("Empty")}
        {:else}
          {$t("All tokens less than")} ${filterTokenType.value}
        {/if}
      </div>
    {:else}
      <VirtualList
        scrollDirection="vertical"
        width="100%"
        height={data.length < 10 ? data.length * 380 : 940}
        itemCount={data.length}
        itemSize={380}
      >
        <div
          class="flex flex-col gap-4 border-b-[1px] border_0000000d last:border-none py-4"
          slot="item"
          let:index
          let:style
          {style}
        >
          <div class="flex justify-between items-start">
            <div class="relative flex items-center gap-3">
              <div class="relative">
                <div class="rounded-full w-[30px] h-[30px] overflow-hidden">
                  <Image logo={data[index].logo} defaultLogo={defaultToken} />
                </div>
                {#if ["EVM", "MOVE", "SOL", "BUNDLE"].includes($typeWallet)}
                  <div class="absolute -top-2 -right-1">
                    <img
                      src={detectedChain(data[index]?.chain)?.logo}
                      alt=""
                      width="15"
                      height="15"
                      class="rounded-full"
                    />
                  </div>
                {/if}
              </div>

              <div class="flex flex-col">
                <div class="font-medium text-base">
                  {#if !data[index]?.name || data[index]?.name === "N/A"}
                    <span>
                      {shorterAddress(data[index]?.contractAddress)}
                    </span>
                  {:else}
                    <span>
                      {data[index]?.name}
                    </span>
                  {/if}
                </div>

                <div class="flex items-center gap-3 min-h-[26px]">
                  <div class="relative font-medium text_00000080 text-xs">
                    {#if !data[index]?.symbol || data[index]?.symbol === "N/A" || data[index].symbol === undefined}
                      N/A
                    {:else}
                      {data[index].symbol}
                    {/if}
                  </div>

                  {#if data[index]?.positionType === "ERC_404"}
                    <span
                      class="inline-flex items-center gap-x-1.5 rounded-full bg-yellow-100 px-1 py-0.5 text-[10px] font-medium text-yellow-800"
                    >
                      <svg
                        class="h-1.5 w-1.5 fill-yellow-500"
                        viewBox="0 0 6 6"
                        aria-hidden="true"
                      >
                        <circle cx={3} cy={3} r={3} />
                      </svg>
                      ERC 404
                    </span>
                  {/if}

                  {#if data[index]?.is_spam}
                    <div
                      class="text-[10px] rounded-[30px] px-2 py-1 w-max mt-1"
                      style="background-color: #FF5733; color: #FFD3B6;"
                    >
                      SCAM
                    </div>
                  {/if}
                </div>
              </div>
            </div>

            <div
              class="relative cursor-pointer"
              on:click={(e) => {
                e.stopPropagation();
                isShowReportTable = true;
                selectedTokenDetail.update((n) => (n = data[index]));
              }}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="18"
                height="18"
                viewBox="0 0 24 24"
              >
                <g
                  fill="none"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                  stroke-width="2"
                >
                  <path d="M0 0h24v24H0z" />
                  <path
                    fill="currentColor"
                    d="M19 4c.852 0 1.297.986.783 1.623l-.076.084L15.915 9.5l3.792 3.793c.603.602.22 1.614-.593 1.701L19 15H6v6a1 1 0 0 1-.883.993L5 22a1 1 0 0 1-.993-.883L4 21V5a1 1 0 0 1 .883-.993L5 4h14z"
                  />
                </g>
              </svg>
              {#if isShowTooltipReport && selectedItemIndex === index}
                <div
                  class="absolute -top-8 left-1/2 transform -translate-x-1/2"
                  style="z-index: 2147483648;"
                >
                  <Tooltip text="Report" />
                </div>
              {/if}
            </div>
          </div>

          <!-- {#if handleCheckVault(data[index]) !== undefined && isNimbusVersion}
            <div
              class="-mt-1 w-max flex items-center justyfy-center px-2 py-1 text_27326F text-xs font-medium bg-[#1e96fc33] rounded-[1000px] cursor-pointer"
              on:click={() => {
                showTableVaults = true;
                selectedVaults = data[index].vaults;
                selectedVaultsSymbol = data[index].symbol;
                selectedTokenDetail.update((n) => (n = data[index]));
              }}
            >
              <div class="flex gap-1">
                Farm up to
                {numeral(handleCheckVault(data[index])?.apy * 100).format(
                  "0,0.00",
                )}% APY
              </div>
            </div>
          {/if} -->

          <div class="flex justify-between items-start">
            <div class="text-right text-sm uppercase font-medium">
              {$t("Price/24h Change")}
            </div>
            <div class="flex flex-col items-end">
              <div
                class="flex items-center justify-end font-medium text-sm text_00000099"
              >
                $<TooltipNumber
                  number={Number(data[index].market_price)}
                  type="balance"
                />
              </div>
              <div
                class="flex items-center justify-end font-medium text-sm gap-1"
              >
                {#if data[index].percent_24h_price_change === null || data[index].percent_24h_price_change === 0}
                  <div class="text_00000099">-</div>
                {:else}
                  <div
                    class={`${
                      data[index].percent_24h_price_change >= 0
                        ? "text-[#00A878]"
                        : "text-red-500"
                    }`}
                  >
                    <TooltipNumber
                      number={Math.abs(data[index].percent_24h_price_change)}
                      type={Math.abs(data[index].percent_24h_price_change) >
                        999999 ||
                      Math.abs(data[index].percent_24h_price_change) < 0.001
                        ? "balance"
                        : "percent"}
                    />
                    <span>%</span>
                  </div>
                  <img
                    src={data[index].percent_24h_price_change !== 0
                      ? data[index].percent_24h_price_change >= 0
                        ? TrendUp
                        : TrendDown
                      : ""}
                    alt="trend"
                    class="mb-1"
                  />
                {/if}
              </div>
            </div>
          </div>

          <div class="flex justify-between items-start">
            <div class="text-right text-sm uppercase font-medium">
              {$t("Amount")}
            </div>
            <div
              class="flex items-center justify-end gap-1 font-medium text-sm text_00000099"
            >
              {#if handleCheckWithinLast24Hours(data[index]) < 24 && handleCheckWithinLast24Hours(data[index]) > 0}
                <span
                  use:tooltip={{
                    content: `<tooltip-detail text="Changed ${handleCheckWithinLast24Hours(data[index])} hrs ago" />`,
                    allowHTML: true,
                    placement: "top",
                    interactive: true,
                  }}
                  class="cursor-pointer"
                >
                  <div class="w-2 h-2 bg-indigo-500 rounded-full" />
                </span>
              {/if}
              <TooltipNumber
                number={data[index]?.amount || 0}
                type="balance"
                personalValue
              />
            </div>
          </div>

          <div class="flex justify-between items-start">
            <div class="text-right text-sm uppercase font-medium">
              {$t("Value")}
            </div>
            <div class="flex flex-col gap-1">
              <div class="flex justify-end font-medium text-sm text_00000099">
                <TooltipNumber
                  number={handleCalculateValue(
                    data[index],
                    Number(data[index].market_price),
                  )}
                  type="value"
                  personalValue
                />
              </div>
              <!-- <div class="flex flex-col items-end justify-end gap-1">
                <div class="flex justify-end text-gray-400 text-sm">
                  <TooltipNumber
                    number={handleCalculateRatio(
                      data[index],
                      Number(data[index].market_price),
                    )}
                    type="percent"
                  />%
                </div>

                <div class="w-3/4 max-w-20">
                  <Progressbar
                    progress={handleCalculateRatio(
                      data[index],
                      Number(data[index].market_price),
                    )}
                    animate
                    size="h-1"
                  />
                </div>
              </div> -->
            </div>
          </div>

          <div class="flex justify-between items-start">
            <div class="text-right text-sm uppercase font-medium">PnL</div>
            <div
              class="view-pnl flex items-center justify-end gap-1 font-medium text-sm"
            >
              {#if ["BTC"].includes($typeWallet)}
                N/A
              {:else}
                <div class="flex flex-col">
                  <div
                    class={`flex justify-end ${
                      handleCalculatePnL(data[index])?.pnl !== 0
                        ? handleCalculatePnL(data[index])?.pnl >= 0
                          ? "text-[#00A878]"
                          : "text-red-500"
                        : "text_00000099"
                    }`}
                  >
                    <TooltipNumber
                      number={Math.abs(handleCalculatePnL(data[index])?.pnl)}
                      type="value"
                      personalValue
                    />
                  </div>
                  <div class="flex items-center justify-end gap-1">
                    <div
                      class={`flex items-center ${
                        handleCalculatePnL(data[index])?.pnl !== 0
                          ? handleCalculatePnL(data[index])?.pnl >= 0
                            ? "text-[#00A878]"
                            : "text-red-500"
                          : "text_00000099"
                      }`}
                    >
                      <TooltipNumber
                        number={Math.abs(
                          handleCalculatePnL(data[index])?.percentPnL,
                        ) * 100}
                        type={Math.abs(
                          handleCalculatePnL(data[index])?.percentPnL,
                        ) *
                          100 >
                        999999
                          ? "balance"
                          : "percent"}
                      />
                      <span>%</span>
                    </div>
                    {#if handleCalculatePnL(data[index])?.pnl !== 0}
                      <img
                        src={handleCalculatePnL(data[index])?.pnl >= 0
                          ? TrendUp
                          : TrendDown}
                        alt="trend"
                        class="mb-1"
                      />
                    {/if}
                  </div>
                </div>
              {/if}
            </div>
          </div>

          <div class="flex justify-between items-center">
            <div class="text-right text-sm uppercase font-medium mt-1">
              {$t("Action")}
            </div>
            <div class="flex items-center gap-4">
              {#if $typeWallet === "BUNDLE"}
                <div
                  class="mt-1"
                  on:click={() => {
                    showTokenInfoBundle = true;
                    selectedTokenDetail.update((n) => (n = data[index]));
                  }}
                >
                  <svg
                    width="28"
                    height="28"
                    viewBox="0 0 20 20"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      fill={$isDarkMode ? "white" : "#00000080"}
                      d="M3.5 4A1.5 1.5 0 0 0 2 5.5v2A1.5 1.5 0 0 0 3.5 9h2A1.5 1.5 0 0 0 7 7.5v-2A1.5 1.5 0 0 0 5.5 4h-2ZM3 5.5a.5.5 0 0 1 .5-.5h2a.5.5 0 0 1 .5.5v2a.5.5 0 0 1-.5.5h-2a.5.5 0 0 1-.5-.5v-2ZM9.5 5a.5.5 0 0 0 0 1h8a.5.5 0 0 0 0-1h-8Zm0 2a.5.5 0 0 0 0 1h6a.5.5 0 0 0 0-1h-6Zm-6 4A1.5 1.5 0 0 0 2 12.5v2A1.5 1.5 0 0 0 3.5 16h2A1.5 1.5 0 0 0 7 14.5v-2A1.5 1.5 0 0 0 5.5 11h-2ZM3 12.5a.5.5 0 0 1 .5-.5h2a.5.5 0 0 1 .5.5v2a.5.5 0 0 1-.5.5h-2a.5.5 0 0 1-.5-.5v-2Zm6.5-.5a.5.5 0 0 0 0 1h8a.5.5 0 0 0 0-1h-8Zm0 2a.5.5 0 0 0 0 1h6a.5.5 0 0 0 0-1h-6Z"
                    />
                  </svg>
                </div>
              {/if}
              {#if listSupported.includes($typeWallet)}
                <div
                  class="flex justify-center cursor-pointer"
                  on:click={() => {
                    showSideTokenDetail = true;
                    selectedTokenDetail.update((n) => (n = data[index]));
                  }}
                >
                  <div class="w-[20px] h-[20px]">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      xmlns:xlink="http://www.w3.org/1999/xlink"
                      version="1.1"
                      viewBox="0 0 256 256"
                      xml:space="preserve"
                    >
                      <defs />
                      <g
                        style="stroke: none; stroke-width: 0; stroke-dasharray: none; stroke-linecap: butt; stroke-linejoin: miter; stroke-miterlimit: 10; fill-rule: nonzero; opacity: 1;"
                        fill={$isDarkMode ? "white" : "#00000080"}
                        transform="translate(1.4065934065934016 1.4065934065934016) scale(2.81 2.81)"
                      >
                        <path
                          d="M 87.994 0 H 69.342 c -1.787 0 -2.682 2.16 -1.418 3.424 l 5.795 5.795 l -33.82 33.82 L 28.056 31.196 l -3.174 -3.174 c -1.074 -1.074 -2.815 -1.074 -3.889 0 L 0.805 48.209 c -1.074 1.074 -1.074 2.815 0 3.889 l 3.174 3.174 c 1.074 1.074 2.815 1.074 3.889 0 l 15.069 -15.069 l 14.994 14.994 c 1.074 1.074 2.815 1.074 3.889 0 l 1.614 -1.614 c 0.083 -0.066 0.17 -0.125 0.247 -0.202 l 37.1 -37.1 l 5.795 5.795 C 87.84 23.34 90 22.445 90 20.658 V 2.006 C 90 0.898 89.102 0 87.994 0 z"
                          style="stroke: none; stroke-width: 1; stroke-dasharray: none; stroke-linecap: butt; stroke-linejoin: miter; stroke-miterlimit: 10; fill-rule: nonzero; opacity: 1;"
                          transform=" matrix(1 0 0 1 0 0) "
                          fill={$isDarkMode ? "white" : "#00000080"}
                          stroke-linecap="round"
                        />
                        <path
                          d="M 65.626 37.8 v 49.45 c 0 1.519 1.231 2.75 2.75 2.75 h 8.782 c 1.519 0 2.75 -1.231 2.75 -2.75 V 23.518 L 65.626 37.8 z"
                          style="stroke: none; stroke-width: 1; stroke-dasharray: none; stroke-linecap: butt; stroke-linejoin: miter; stroke-miterlimit: 10; fill-rule: nonzero; opacity: 1;"
                          fill={$isDarkMode ? "white" : "#00000080"}
                          transform=" matrix(1 0 0 1 0 0) "
                          stroke-linecap="round"
                        />
                        <path
                          d="M 47.115 56.312 V 87.25 c 0 1.519 1.231 2.75 2.75 2.75 h 8.782 c 1.519 0 2.75 -1.231 2.75 -2.75 V 42.03 L 47.115 56.312 z"
                          style="stroke: none; stroke-width: 1; stroke-dasharray: none; stroke-linecap: butt; stroke-linejoin: miter; stroke-miterlimit: 10; fill-rule: nonzero; opacity: 1;"
                          fill={$isDarkMode ? "white" : "#00000080"}
                          transform=" matrix(1 0 0 1 0 0) "
                          stroke-linecap="round"
                        />
                        <path
                          d="M 39.876 60.503 c -1.937 0 -3.757 -0.754 -5.127 -2.124 l -6.146 -6.145 V 87.25 c 0 1.519 1.231 2.75 2.75 2.75 h 8.782 c 1.519 0 2.75 -1.231 2.75 -2.75 V 59.844 C 41.952 60.271 40.933 60.503 39.876 60.503 z"
                          style="stroke: none; stroke-width: 1; stroke-dasharray: none; stroke-linecap: butt; stroke-linejoin: miter; stroke-miterlimit: 10; fill-rule: nonzero; opacity: 1;"
                          fill={$isDarkMode ? "white" : "#00000080"}
                          transform=" matrix(1 0 0 1 0 0) "
                          stroke-linecap="round"
                        />
                        <path
                          d="M 22.937 46.567 L 11.051 58.453 c -0.298 0.298 -0.621 0.562 -0.959 0.8 V 87.25 c 0 1.519 1.231 2.75 2.75 2.75 h 8.782 c 1.519 0 2.75 -1.231 2.75 -2.75 V 48.004 L 22.937 46.567 z"
                          style="stroke: none; stroke-width: 1; stroke-dasharray: none; stroke-linecap: butt; stroke-linejoin: miter; stroke-miterlimit: 10; fill-rule: nonzero; opacity: 1;"
                          fill={$isDarkMode ? "white" : "#00000080"}
                          transform=" matrix(1 0 0 1 0 0) "
                          stroke-linecap="round"
                        />
                      </g>
                    </svg>
                  </div>
                </div>
              {/if}
              {#if listChainSupportedSwap.includes(data[index]?.chain)}
                <div class="flex justify-center">
                  <div
                    class="cursor-pointer transform rotate-90"
                    on:click={() => {
                      showSideTokenSwap = true;
                      selectedTokenDetail.update((n) => (n = data[index]));
                    }}
                  >
                    <svg
                      width="28"
                      height="28"
                      viewBox="0 0 21 22"
                      fill={$isDarkMode ? "white" : "#00000080"}
                      xmlns="http://www.w3.org/2000/svg"
                      ><path
                        d="M6.51043 7.47998V14.99H7.77043V7.47998L9.66043 9.36998L10.5505 8.47994L7.5859 5.51453C7.3398 5.26925 6.94114 5.26925 6.69504 5.51453L3.73047 8.47994L4.62051 9.36998L6.51043 7.47998Z"
                        fill={$isDarkMode ? "white" : "#00000080"}
                      ></path><path
                        d="M14.4902 14.52V7.01001H13.2302V14.52L11.3402 12.63L10.4502 13.5201L13.4148 16.4855C13.6609 16.7308 14.0595 16.7308 14.3056 16.4855L17.2702 13.5201L16.3802 12.63L14.4902 14.52Z"
                        fill={$isDarkMode ? "white" : "#00000080"}
                      ></path></svg
                    >
                  </div>
                </div>
              {/if}
            </div>
          </div>
        </div>
      </VirtualList>
    {/if}

    {#if isLoading}
      <div class="flex justify-center items-center h-full py-3 px-3">
        {#if isNimbusVersion}
          <LoadingPremium />
        {:else}
          <Loading />
        {/if}
      </div>
    {/if}
  </span>
</Table>

<!-- Modal token holding information when bundle -->
<AppOverlay
  clickOutSideToClose
  isOpen={showTokenInfoBundle}
  isTableContent
  on:close={() => {
    showTokenInfoBundle = false;
    selectedTokenDetail.update((n) => (n = {}));
  }}
>
  <div class="h-[563px] flex flex-col gap-4">
    <div class="font-medium text-2xl">
      {$t("Token breakdown")}
    </div>
    <div
      class={`rounded-[10px] overflow-visible overflow-y-auto h-[563px] relative ${
        $isDarkMode ? "bg-[#131313]" : "bg-[#fff] border border_0000000d"
      }`}
    >
      <table
        class={`table-auto xl:w-full w-[700px] ${
          handleFormatDataBreakdown($selectedTokenDetail) &&
          handleFormatDataBreakdown($selectedTokenDetail).length === 0
            ? "h-full"
            : ""
        }`}
      >
        <thead>
          <tr class="bg_f4f5f8 sticky left-0 top-0">
            <th class="pl-3 py-3 rounded-tl-[10px]">
              <div class="font-medium text-left uppercase text-sm">
                {$t("Account")}
              </div>
            </th>
            <th class="py-3">
              <div class="font-medium text-right uppercase text-sm">
                {$t("Amount")}
              </div>
            </th>
            <th class="py-3 pr-3 rounded-tr-[10px]">
              <div class="font-medium text-right uppercase text-sm">
                {$t("Value")}
              </div>
            </th>
          </tr>
        </thead>
        <tbody>
          {#if handleFormatDataBreakdown($selectedTokenDetail) && handleFormatDataBreakdown($selectedTokenDetail).length === 0}
            <tr>
              <td colspan="3">
                <div
                  class="flex items-center justify-center px-3 py-3 text-base text-gray-400"
                >
                  {$t("Empty")}
                </div>
              </td>
            </tr>
          {:else}
            {#each handleFormatDataBreakdown($selectedTokenDetail) as item}
              <tr class="transition-all cursor-pointer group">
                <td
                  class={`pl-3 py-3 ${
                    $isDarkMode
                      ? "group-hover:bg-[#000]"
                      : "group-hover:bg-gray-100"
                  }`}
                >
                  <div class="flex items-center gap-3">
                    <div class="rounded-full w-[30px] h-[30px] overflow-hidden">
                      <Image logo={data.logo} defaultLogo={defaultToken} />
                    </div>
                    <div class="flex flex-col items-start">
                      <div class="font-medium text-sm text_00000099">
                        {item.label}
                      </div>
                      <div class="text-sm flex justify-end items-center">
                        <Copy
                          address={item?.owner}
                          iconColor={$isDarkMode ? "#fff" : "#000"}
                          color={$isDarkMode ? "#fff" : "#000"}
                          isShorten
                        />
                      </div>
                    </div>
                  </div>
                </td>

                <td
                  class={`py-3 ${
                    $isDarkMode
                      ? "group-hover:bg-[#000]"
                      : "group-hover:bg-gray-100"
                  }`}
                >
                  <div
                    class="text-sm text_00000099 flex flex-col justify-end items-end gap-2"
                  >
                    <div
                      class="flex justify-end items-center gap-1 font-medium text-sm text_00000099"
                    >
                      <TooltipNumber
                        number={item?.amount || 0}
                        type="balance"
                      />
                      {#if $selectedTokenDetail.symbol === undefined}
                        N/A
                      {:else}
                        {$selectedTokenDetail.symbol}
                      {/if}
                    </div>
                    <div
                      class="flex flex-col items-end justify-end gap-1 w-full"
                    >
                      <div class="flex justify-end text-gray-400 text-sm">
                        <TooltipNumber
                          number={Math.abs(
                            item?.amount / $selectedTokenDetail?.amount,
                          ) * 100}
                          type={Math.abs(
                            item?.amount / $selectedTokenDetail?.amount,
                          ) *
                            100 >
                          999999
                            ? "balance"
                            : "percent"}
                        />%
                      </div>
                      <div class="w-3/4 max-w-30">
                        <Progressbar
                          progress={Math.abs(
                            item?.amount / $selectedTokenDetail?.amount,
                          ) * 100}
                          size="h-1"
                        />
                      </div>
                    </div>
                  </div>
                </td>

                <td
                  class={`py-3 pr-3 ${
                    $isDarkMode
                      ? "group-hover:bg-[#000]"
                      : "group-hover:bg-gray-100"
                  }`}
                >
                  <div
                    class="text-sm text_00000099 font-medium flex justify-end items-center"
                  >
                    $<TooltipNumber
                      number={Number(item?.amount) *
                        Number($selectedTokenDetail?.market_price)}
                      type="balance"
                    />
                  </div>
                </td>
              </tr>
            {/each}
          {/if}
        </tbody>
      </table>
    </div>
  </div>
</AppOverlay>

<!-- Modal vault -->
<AppOverlay
  clickOutSideToClose
  isOpen={showTableVaults}
  isTableContent
  on:close={() => {
    showTableVaults = false;
    selectedTokenDetail.update((n) => (n = {}));
  }}
>
  <VaultTable data={selectedVaults} symbol={selectedVaultsSymbol} />
</AppOverlay>

<AppOverlay
  clickOutSideToClose
  isOpen={$showScallopDeFiFarmModal}
  on:close={() => {
    showScallopDeFiFarmModal.update((n) => (n = false));
  }}
>
  {#if $showScallopDeFiFarmModal}
    <ScallopProvideLiquidity />
  {/if}
</AppOverlay>

<!-- Modal report spam/trash token  -->
<AppOverlay
  clickOutSideToClose
  isOpen={isShowReportTable &&
    $selectedTokenDetail &&
    Object.keys($selectedTokenDetail)?.length !== 0}
  on:close={() => {
    isShowReportTable = false;
    isOldToken = false;
    selectedTokenDetail.update((n) => (n = {}));
  }}
>
  <div class="flex flex-col gap-4 xl:mt-0 mt-4">
    <div class="font-medium title-3">
      {$t("Blacklist Token")}
    </div>
    <form
      on:submit|preventDefault={handleReportTrashCoin}
      class="flex flex-col xl:gap-3 gap-6"
    >
      <div class="flex flex-col gap-6 xl:gap-3">
        <div
          class={`flex flex-col gap-1 input-2 input-border w-full py-[6px] px-3 ${
            !$isDarkMode ? "bg-[#F0F2F7]" : "bg_fafafbff"
          }`}
        >
          <div
            class={`text-base font-medium ${
              $isDarkMode ? "text-gray-400" : "text-[#666666]"
            }`}
          >
            {$t("Chain")}
          </div>
          <input
            type="text"
            id="chain"
            name="chain"
            value={$selectedTokenDetail?.chain}
            class={`p-0 border-none focus:outline-none focus:ring-0 text-sm font-normal ${
              !$isDarkMode ? "bg-[#F0F2F7]" : "bg-transparent"
            } ${
              $isDarkMode
                ? "text-white"
                : "text-[#5E656B] placeholder-[#5E656B]"
            }`}
            disabled
          />
        </div>

        <div
          class={`flex flex-col gap-1 input-2 input-border w-full py-[6px] px-3 ${
            !$isDarkMode ? "bg-[#F0F2F7]" : "bg_fafafbff"
          }`}
        >
          <div
            class={`text-base font-medium ${
              $isDarkMode ? "text-gray-400" : "text-[#666666]"
            }`}
          >
            {$t("Contract Address")}
          </div>
          <input
            type="text"
            id="contract_address"
            name="contract_address"
            value={$selectedTokenDetail?.contractAddress}
            class={`p-0 border-none focus:outline-none focus:ring-0 text-sm font-normal ${
              !$isDarkMode ? "bg-[#F0F2F7]" : "bg-transparent"
            } ${
              $isDarkMode
                ? "text-white"
                : "text-[#5E656B] placeholder-[#5E656B]"
            }`}
            disabled
          />
        </div>

        <div
          class={`flex flex-col gap-3 input-2 input-border w-full py-[8px] px-3 ${
            (reportReasonList.length !== 0 || isOldToken) && !$isDarkMode
              ? "bg-[#F0F2F7]"
              : "bg_fafafbff"
          }`}
        >
          <div class="text-base text-[#666666] font-medium">
            {$t("Reason")}
          </div>
          {#each reasonReport as item}
            <div class="flex items-center gap-2 cursor-pointer w-max">
              <input
                type="checkbox"
                name={item.id}
                id={item.id}
                value={item.id}
                bind:group={reportReasonList}
                class="cursor-pointer relative w-5 h-5 appearance-none rounded-[0.25rem] border outline-none before:pointer-events-none before:absolute before:h-[0.875rem] before:w-[0.875rem] before:scale-0 before:rounded-full before:bg-transparent before:opacity-0 before:shadow-[0px_0px_0px_13px_transparent] before:content-[''] checked:border-primary checked:bg-primary checked:before:opacity-[0.16] checked:after:absolute checked:after:-mt-px checked:after:ml-[0.25rem] checked:after:block checked:after:h-[0.8125rem] checked:after:w-[0.375rem] checked:after:rotate-45 checked:after:border-[0.125rem] checked:after:border-l-0 checked:after:border-t-0 checked:after:border-solid checked:after:border-white checked:after:bg-transparent checked:after:content-[''] hover:cursor-pointer hover:before:opacity-[0.04] hover:before:shadow-[0px_0px_0px_13px_rgba(0,0,0,0.6)] focus:shadow-none focus:transition-[border-color_0.2s] focus:before:scale-100 focus:before:opacity-[0.12] focus:before:shadow-[0px_0px_0px_13px_rgba(0,0,0,0.6)] focus:before:transition-[box-shadow_0.2s,transform_0.2s] focus:after:absolute focus:after:z-[1] focus:after:block focus:after:h-[0.875rem] focus:after:w-[0.875rem] focus:after:rounded-[0.125rem] focus:after:content-[''] checked:focus:before:scale-100 checked:focus:before:shadow-[0px_0px_0px_13px_#3b71ca] checked:focus:before:transition-[box-shadow_0.2s,transform_0.2s] checked:focus:after:-mt-px checked:focus:after:ml-[0.25rem] checked:focus:after:h-[0.8125rem] checked:focus:after:w-[0.375rem] checked:focus:after:rotate-45 checked:focus:after:rounded-none checked:focus:after:border-[0.125rem] checked:focus:after:border-l-0 checked:focus:after:border-t-0 checked:focus:after:border-solid checked:focus:after:border-white checked:focus:after:bg-transparent dark:border-neutral-600 dark:checked:border-primary dark:checked:bg-primary dark:focus:before:shadow-[0px_0px_0px_13px_rgba(255,255,255,0.4)] dark:checked:focus:before:shadow-[0px_0px_0px_13px_#3b71ca]"
              />
              <label
                for={item.id}
                class="text-sm font-normal text-[#5E656B] cursor-pointer"
              >
                {$t(item.content)}
              </label>
            </div>
          {/each}

          <div class="flex items-center gap-2 cursor-pointer w-max">
            <input
              type="checkbox"
              name="outdated"
              id="outdated"
              on:change={(event) => {
                isOldToken = event.target.checked;
              }}
              class="cursor-pointer relative w-5 h-5 appearance-none rounded-[0.25rem] border outline-none before:pointer-events-none before:absolute before:h-[0.875rem] before:w-[0.875rem] before:scale-0 before:rounded-full before:bg-transparent before:opacity-0 before:shadow-[0px_0px_0px_13px_transparent] before:content-[''] checked:border-primary checked:bg-primary checked:before:opacity-[0.16] checked:after:absolute checked:after:-mt-px checked:after:ml-[0.25rem] checked:after:block checked:after:h-[0.8125rem] checked:after:w-[0.375rem] checked:after:rotate-45 checked:after:border-[0.125rem] checked:after:border-l-0 checked:after:border-t-0 checked:after:border-solid checked:after:border-white checked:after:bg-transparent checked:after:content-[''] hover:cursor-pointer hover:before:opacity-[0.04] hover:before:shadow-[0px_0px_0px_13px_rgba(0,0,0,0.6)] focus:shadow-none focus:transition-[border-color_0.2s] focus:before:scale-100 focus:before:opacity-[0.12] focus:before:shadow-[0px_0px_0px_13px_rgba(0,0,0,0.6)] focus:before:transition-[box-shadow_0.2s,transform_0.2s] focus:after:absolute focus:after:z-[1] focus:after:block focus:after:h-[0.875rem] focus:after:w-[0.875rem] focus:after:rounded-[0.125rem] focus:after:content-[''] checked:focus:before:scale-100 checked:focus:before:shadow-[0px_0px_0px_13px_#3b71ca] checked:focus:before:transition-[box-shadow_0.2s,transform_0.2s] checked:focus:after:-mt-px checked:focus:after:ml-[0.25rem] checked:focus:after:h-[0.8125rem] checked:focus:after:w-[0.375rem] checked:focus:after:rotate-45 checked:focus:after:rounded-none checked:focus:after:border-[0.125rem] checked:focus:after:border-l-0 checked:focus:after:border-t-0 checked:focus:after:border-solid checked:focus:after:border-white checked:focus:after:bg-transparent dark:border-neutral-600 dark:checked:border-primary dark:checked:bg-primary dark:focus:before:shadow-[0px_0px_0px_13px_rgba(255,255,255,0.4)] dark:checked:focus:before:shadow-[0px_0px_0px_13px_#3b71ca]"
            />
            <label
              for="outdated"
              class="text-sm font-normal text-[#5E656B] cursor-pointer"
            >
              {$t("The token is outdate")}
            </label>
          </div>

          {#if isOldToken}
            <textarea
              placeholder="Please type info about that token"
              rows="5"
              id="reason"
              name="reason"
              class={`mb-2 p-0 input-2 input-border w-full py-[6px] px-3 focus:outline-none focus:ring-0 text-sm font-normal ${
                !$isDarkMode ? "bg-[#F0F2F7]" : "bg-transparent"
              } ${
                $isDarkMode
                  ? "text-white"
                  : "text-[#5E656B] placeholder-[#5E656B]"
              }`}
            />
          {/if}
        </div>

        <div class="flex justify-end gap-4">
          <div class="w-[120px]">
            <Button
              variant="secondary"
              on:click={() => {
                isShowReportTable = false;
                isOldToken = false;
                selectedTokenDetail.update((n) => (n = {}));
              }}
            >
              {$t("Cancel")}
            </Button>
          </div>
          <div class="w-[120px]">
            <Button
              type="submit"
              variant="tertiary"
              isLoading={isLoadingReportTrashCoin}
              disabled={isLoadingReportTrashCoin}
            >
              {$t("Report")}
            </Button>
          </div>
        </div>
      </div>
    </form>
  </div>
</AppOverlay>

<!-- Sidebar Token Detail -->
<OverlaySidebar isOpen={showSideTokenDetail}>
  <div class="flex justify-between items-start">
    <div
      class="text-5xl text-gray-500 cursor-pointer"
      on:click|stopPropagation={() => {
        showSideTokenDetail = false;
        selectedTokenDetail.update((n) => (n = {}));
      }}
    >
      &times;
    </div>
    {#if $selectedTokenDetail && Object.keys($selectedTokenDetail).length !== 0}
      <div class="flex flex-col gap-2">
        <div class="flex items-center gap-4">
          <div class="relative">
            <div class="rounded-full w-[46px] h-[46px] overflow-hidden">
              <Image
                logo={$selectedTokenDetail?.logo}
                defaultLogo={defaultToken}
              />
            </div>
            {#if ["EVM", "MOVE", "SOL", "BUNDLE"].includes($typeWallet)}
              <div class="absolute -top-2 -right-1">
                <img
                  src={detectedChain($selectedTokenDetail?.chain)?.logo}
                  alt=""
                  width="26"
                  height="26"
                  class="rounded-full"
                />
              </div>
            {/if}
          </div>
          <div class="flex flex-col">
            <div class="flex items-start gap-2">
              <div
                class="relative font-medium text-xl"
                on:mouseover={() => {
                  isShowTooltipName = true;
                }}
                on:mouseleave={() => (isShowTooltipName = false)}
              >
                {#if $selectedTokenDetail.name === undefined}
                  N/A
                {:else}
                  {$selectedTokenDetail?.name?.length > 20
                    ? shorterName($selectedTokenDetail.name, 20)
                    : $selectedTokenDetail.name}
                {/if}
                {#if isShowTooltipName && $selectedTokenDetail?.name?.length > 20}
                  <div
                    class="absolute left-0 -top-8"
                    style="z-index: 2147483648;"
                  >
                    <Tooltip text={$selectedTokenDetail.name} />
                  </div>
                {/if}
              </div>
            </div>

            <div class="flex items-center gap-2">
              <div
                class="relative font-medium text_00000080 text-base"
                on:mouseover={() => {
                  isShowTooltipSymbol = true;
                }}
                on:mouseleave={() => (isShowTooltipSymbol = false)}
              >
                {#if $selectedTokenDetail.symbol === undefined}
                  N/A
                {:else}
                  {shorterName($selectedTokenDetail.symbol, 20)}
                {/if}
                {#if isShowTooltipSymbol && $selectedTokenDetail.symbol.length > 20}
                  <div
                    class="absolute left-0 -top-8"
                    style="z-index: 2147483648;"
                  >
                    <Tooltip text={$selectedTokenDetail?.symbol} />
                  </div>
                {/if}
              </div>
              <CopyToClipboard
                text={$selectedTokenDetail?.contractAddress === "TONCOIN"
                  ? "EQAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAM9c"
                  : $selectedTokenDetail?.contractAddress}
                let:copy
                on:copy={async () => {
                  isCopied = true;
                  await wait(1000);
                  isCopied = false;
                }}
              >
                <div
                  class="cursor-pointer relative"
                  on:mouseover={() => {
                    isShowTooltipContractAddress = true;
                  }}
                  on:mouseleave={() => (isShowTooltipContractAddress = false)}
                  on:click={copy}
                >
                  {#if isCopied}
                    <svg
                      width={20}
                      height={20}
                      id="Layer_1"
                      data-name="Layer 1"
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 122.88 74.46"
                      fill={$isDarkMode ? "#d1d5db" : "#00000080"}
                      ><path
                        fill-rule="evenodd"
                        d="M1.87,47.2a6.33,6.33,0,1,1,8.92-9c8.88,8.85,17.53,17.66,26.53,26.45l-3.76,4.45-.35.37a6.33,6.33,0,0,1-8.95,0L1.87,47.2ZM30,43.55a6.33,6.33,0,1,1,8.82-9.07l25,24.38L111.64,2.29c5.37-6.35,15,1.84,9.66,8.18L69.07,72.22l-.3.33a6.33,6.33,0,0,1-8.95.12L30,43.55Zm28.76-4.21-.31.33-9.07-8.85L71.67,4.42c5.37-6.35,15,1.83,9.67,8.18L58.74,39.34Z"
                      /></svg
                    >
                  {:else}
                    <svg
                      width={20}
                      height={20}
                      viewBox="0 0 12 11"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M8.1875 3.3125H10.6875V10.1875H3.8125V7.6875"
                        stroke={$isDarkMode ? "#d1d5db" : "#00000080"}
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      />
                      <path
                        d="M8.1875 0.8125H1.3125V7.6875H8.1875V0.8125Z"
                        stroke={$isDarkMode ? "#d1d5db" : "#00000080"}
                        stroke-linecap="round"
                        stroke-linejoin="round"
                      />
                    </svg>
                  {/if}

                  {#if isShowTooltipContractAddress}
                    <div
                      class="absolute right-0 -top-8"
                      style="z-index: 2147483648;"
                    >
                      <Tooltip
                        text={shorterAddress(
                          $selectedTokenDetail?.contractAddress === "TONCOIN"
                            ? "EQAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAM9c"
                            : $selectedTokenDetail?.contractAddress,
                        )}
                      />
                    </div>
                  {/if}
                </div>
              </CopyToClipboard>
            </div>
          </div>
        </div>
        <div class="flex items-center font-medium text-2xl">
          $<TooltipNumber
            number={Number($selectedTokenDetail?.market_price)}
            type="balance"
          />
        </div>
      </div>
    {/if}
  </div>

  {#if showSideTokenDetail}
    <TokenDetailSidebar data={$selectedTokenDetail} {showSideTokenDetail} />
  {/if}
</OverlaySidebar>

<!-- Sidebar Token Swap -->
<OverlaySidebarSwap isOpen={showSideTokenSwap}>
  <div class="flex justify-between items-start">
    <div
      class="text-5xl text-gray-500 cursor-pointer"
      on:click|stopPropagation={() => {
        showSideTokenSwap = false;
        selectedTokenDetail.update((n) => (n = {}));
      }}
    >
      &times;
    </div>
    {#if $selectedTokenDetail && Object.keys($selectedTokenDetail).length !== 0}
      <div class="flex items-center justify-end gap-4">
        <div class="relative">
          <div class="rounded-full w-[46px] h-[46px] overflow-hidden">
            <Image
              logo={$selectedTokenDetail?.logo}
              defaultLogo={defaultToken}
            />
          </div>
          {#if ["EVM", "MOVE", "SOL", "BUNDLE"].includes($typeWallet)}
            <div class="absolute -top-2 -right-1">
              <img
                src={detectedChain($selectedTokenDetail?.chain)?.logo}
                alt=""
                width="26"
                height="26"
                class="rounded-full"
              />
            </div>
          {/if}
        </div>
        <div class="flex flex-col">
          <div class="flex items-start gap-2">
            <div
              class="relative font-medium text-xl"
              on:mouseover={() => {
                isShowTooltipName = true;
              }}
              on:mouseleave={() => (isShowTooltipName = false)}
            >
              {#if $selectedTokenDetail.name === undefined}
                N/A
              {:else}
                {$selectedTokenDetail?.name?.length > 20
                  ? shorterName($selectedTokenDetail.name, 20)
                  : $selectedTokenDetail.name}
              {/if}
              {#if isShowTooltipName && $selectedTokenDetail?.name?.length > 20}
                <div
                  class="absolute left-0 -top-8"
                  style="z-index: 2147483648;"
                >
                  <Tooltip text={$selectedTokenDetail.name} />
                </div>
              {/if}
            </div>
          </div>

          <div class="flex items-center gap-2">
            <div
              class="relative font-medium text_00000080 text-base"
              on:mouseover={() => {
                isShowTooltipSymbol = true;
              }}
              on:mouseleave={() => (isShowTooltipSymbol = false)}
            >
              {#if $selectedTokenDetail.symbol === undefined}
                N/A
              {:else}
                {shorterName($selectedTokenDetail.symbol, 20)}
              {/if}
              {#if isShowTooltipSymbol && $selectedTokenDetail.symbol.length > 20}
                <div
                  class="absolute left-0 -top-8"
                  style="z-index: 2147483648;"
                >
                  <Tooltip text={$selectedTokenDetail.symbol} />
                </div>
              {/if}
            </div>
            <CopyToClipboard
              text={$selectedTokenDetail?.contractAddress}
              let:copy
              on:copy={async () => {
                isCopied = true;
                await wait(1000);
                isCopied = false;
              }}
            >
              <div
                class="cursor-pointer relative"
                on:mouseover={() => {
                  isShowTooltipContractAddress = true;
                }}
                on:mouseleave={() => (isShowTooltipContractAddress = false)}
                on:click={copy}
              >
                {#if isCopied}
                  <svg
                    width={20}
                    height={20}
                    id="Layer_1"
                    data-name="Layer 1"
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 122.88 74.46"
                    fill={$isDarkMode ? "#d1d5db" : "#00000080"}
                    ><path
                      fill-rule="evenodd"
                      d="M1.87,47.2a6.33,6.33,0,1,1,8.92-9c8.88,8.85,17.53,17.66,26.53,26.45l-3.76,4.45-.35.37a6.33,6.33,0,0,1-8.95,0L1.87,47.2ZM30,43.55a6.33,6.33,0,1,1,8.82-9.07l25,24.38L111.64,2.29c5.37-6.35,15,1.84,9.66,8.18L69.07,72.22l-.3.33a6.33,6.33,0,0,1-8.95.12L30,43.55Zm28.76-4.21-.31.33-9.07-8.85L71.67,4.42c5.37-6.35,15,1.83,9.67,8.18L58.74,39.34Z"
                    /></svg
                  >
                {:else}
                  <svg
                    width={20}
                    height={20}
                    viewBox="0 0 12 11"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M8.1875 3.3125H10.6875V10.1875H3.8125V7.6875"
                      stroke={$isDarkMode ? "#d1d5db" : "#00000080"}
                      stroke-linecap="round"
                      stroke-linejoin="round"
                    />
                    <path
                      d="M8.1875 0.8125H1.3125V7.6875H8.1875V0.8125Z"
                      stroke={$isDarkMode ? "#d1d5db" : "#00000080"}
                      stroke-linecap="round"
                      stroke-linejoin="round"
                    />
                  </svg>
                {/if}

                {#if isShowTooltipContractAddress}
                  <div
                    class="absolute right-0 -top-8"
                    style="z-index: 2147483648;"
                  >
                    <Tooltip
                      text={shorterAddress(
                        $selectedTokenDetail?.contractAddress,
                      )}
                    />
                  </div>
                {/if}
              </div>
            </CopyToClipboard>
          </div>
        </div>
      </div>
    {/if}
  </div>

  {#if showSideTokenSwap}
    <Swap />
  {/if}
</OverlaySidebarSwap>

<style>
  :global(body) .bg_fafafbff {
    background: #fafafbff;
  }
  :global(body.dark) .bg_fafafbff {
    background: #212121;
  }
  :global(.virtual-list-wrapper) {
    overflow-x: hidden !important;
  }
</style>
