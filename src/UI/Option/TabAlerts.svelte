<script lang="ts">
  ...
  const handleToggleCheckAll = (e: any) => {
    if (e.target.checked) {
      checkAll = true;
      blacklistAddress = listAddress.map((item: any) => item.address);
    } else {
      blacklistAddress = [];
      checkAll = false;
    }
  ...
  const onSubmitSettingAlert = async () => {
    ...
    const payload = {
      ...
      portfolioSummary: {
        ...
        blacklist: listAddress
          ...
          ?.filter((element: any) => !blacklistAddress.includes(element)),
      },
      transaction: {
        enabled: transaction,
    ...
    try {
      ...
      if (res && res?.data) {
        ...
        triggerToast($t("Your settings"), "success");
      }
    } catch (e) {
      console.error(e);
      // triggerToast(
      ...
      //   "fail",
      // );
      triggerToast($t("There are sme problem"), "fail");
    } finally {
      isLoadingSave = false;
    }
  };
  ...
  const handleSetState = (data: any) => {
    const formatDataBlackList = listAddress
      ...
      ?.filter(
        (item: any) =>
          !data?.alertSettings?.portfolioSummary?.blacklist?.includes(item),
      );
...
<div class="flex flex-col gap-4">
  ...
  >
    <div class="flex flex-col gap-1">
      <div class="xl:title-3 title-2">{$t("Alert Settings")}</div>
      <div class="text-base text-gray-500">
        {$t("Management your setup alerts")}
      </div>
    </div>
    <div
      class="text-base text-blue-500 cursor-pointer hover:underline"
      on:click={() => {
  ...
  >
    <!-- <div class="border-b-[1px] border_0000000d pb-8 flex flex-col gap-4">
      ...
      <div class="flex flex-col gap-8">
        <div class="flex flex-col gap-3">
          <div class="flex justify-between items-center gap-6">
            <div class="flex flex-col">
              <div class="text-base">Price change notification</div>
          ...
          <div class="flex flex-col gap-3">
            {#each percentList as item}
              ...
              >
                ...
                />
                <div class="text-sm font-normal cursor-pointer">
                  {item.content}
                </div>
              </label>
            {/each}
        ...
        <div class="flex flex-col gap-3">
          <div class="flex justify-between items-center gap-6">
            <div class="flex flex-col">
              <div class="text-base">Token holding ignore</div>
              <div class="text-sm text-gray-400">
    ...
    <div class="flex flex-col gap-4">
      <div class="xl:title-4 title-3">{$t("Portfolio summary alerts")}</div>
      <div class="flex flex-col gap-8">
          ...
          <div class="flex flex-col gap-3">
            {#each summaryList as item}
              ...
              >
                ...
                />
                <label for={item.id} class="text-sm font-normal cursor-pointer">
                  {$t(item.content)}
                </label>
              </div>
            {/each}
        ...
        <!-- <div class="flex flex-col gap-4">
          <div class="flex justify-between items-center gap-6">
            <div class="flex flex-col">
              <div class="text-base">Transaction notification</div>
              <div class="text-sm text-gray-400">
    ...
    <div class="flex justify-start gap-4 mt-6">
      <div class="w-[120px]">
        <Button variant="secondary" on:click={cancelSaveSetting}
          >{$t("Cancel")}</Button
        >
      </div>
      <div class="w-[120px]">
...
>
  <div class="flex flex-col gap-4">
    <div class="flex flex-col gap-1">
      ...
      <div class="text-base">{$t("Use the command as follow video")}</div>
    </div>
    <div class="xl:h-[350px] h-max">
      <img
        src={FollowWhale}
        alt=""
        loading="lazy"
