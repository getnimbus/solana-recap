<script lang="ts">
  ...
  import WhitePaperLogo from "~/assets/white-paper-version/white-paper-white.png";
  import WhitePaperLogoBlack from "~/assets/white-paper-version/white-paper-black.png";
</script>

<div
  class={`flex justify-between xl:items-center max-w-[1600px] m-auto w-[94%] xl:flex-row flex-col gap-1 ${isNimbusVersion ? "xl:py-2 py-5" : "xl:py-3 py-6"}`}
>
  <div class="xl:-ml-10 -ml-8 flex items-center">
    {#if isNimbusVersion}
      ...
      <Languages />
    {:else}
      <div class="w-[177px] h-[60px] cursor-pointer relative overflow-hidden">
        <img
          src={$isDarkMode ? WhitePaperLogo : WhitePaperLogoBlack}
          alt="logo"
          loading="lazy"
          decoding="async"
          class="absolute -top-16 left-0 transform scale-125"
        />
      </div>
    {/if}
  </div>
  <div class="flex items-center xl:gap-4 gap-6">
    ...
    {#if isNimbusVersion}
      <a
        ...
        class="text-sm hover:text-[#3b82f6] transition-all"
      >
        GetNimbus.io
      </a>
      ...
      <div class="flex items-center gap-4">
        <div class="text-sm">{$t("Find us on")}</div>
        <div class="flex items-center xl:gap-4 gap-6">
          <a
